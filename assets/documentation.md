...
- **Study: new journal (subject)** — creates a subject folder + index note (Study is a preset, on by default — turn it off in Settings → Journals if you don't use it). These four are hidden from the palette entirely when Study is off, rather than offered and then refused.
...
A title card and three rows. The card at the top is the page's own name with a cog at its right — the cog opens the section editor, and clicking the name renames the note itself. Obsidian's own title above the note is hidden while that card is there, so the name is said once — remove the block and it comes back. **Every Almanac dashboard has this head now**: Search, the diary and journals folder notes, and the four period overviews, each with its own name and a second row of destinations — Home, Diary, Journals — under it. The homepage's own head keeps just the name, because its **Go to** grid is already that row. Diary entries and journal notes have no head: their banner already names the note and renames it. On a folder note the name is the folder's, so a default vault reads **02 - Diary** there, and renaming it renames the folder too. Then the top row is two columns — the diary card on the left, and a grid of shortcuts (**Go to**) with your open tasks and this date in previous years stacked on the right; then the journals card; then the vault's charts. The top row is a single `almanac` block with a `row` line in it, which is what puts those three next to each other. A row wraps rather than squeezing: each cell asks for a minimum width, so the three are three across on a wide window, two and one on a half-width pane, and a plain column on a phone.
