# 📖 ChronoAnvil — how this vault works

This vault is driven by the **ChronoAnvil** plugin, which owns the bespoke "glue" that used to be spread across Templater scripts, Meta Bind buttons, Folder Notes and a set of CSS snippets. The dashboards are now rendered by ChronoAnvil itself, with **Bases** for `.base` views (that remains a dependency). **Dataview, Tracker and Tasks are not required** — the "Open Tasks" lists on the Weekly, Monthly, Staging and Subject dashboards are now the built-in `tasks-table` widget, reading ChronoAnvil's own task lines. The topic tables, confidence summaries, tag index, diary calendars, entry navigator, diary search / on-this-day / timeline and every chart (trend, heatmap, summary, and the study activity chart) are all built-in ChronoAnvil widgets too.

## Required plugins

No **community** plugins are required — the "Open Tasks" lists that would once have needed the Tasks plugin are the built-in `tasks-table` widget.
...
You do **not** need Templater, Meta Bind, CSS snippets, or **Dataview** — the ChronoAnvil plugin replaces all of them, including the topic tables, confidence summaries and tag index, which are now built-in widgets reading Obsidian's own metadata cache directly, same as the diary calendars and entry navigator.
...
is named `<group>: <what it does>`**, so the palette groups them when you
...
Preferred: a fenced `` ```chronoanvil `` block, one directive per line. Simple widgets on consecutive lines (slider/time/date/select/button) collapse into a single row; anything else — `diary`, `month-summary`, `journals`, `timeline` — renders as its own full-width block:
...
- `month-summary` — the Monthly Overview dashboard's whole summary, driven by that note's `month-start` property, as one card: an accent-washed **banner** at the top — the "Monthly Overview" eyebrow, the month as its title, the stats line, and the month navigator — welded above the day grid and the year-of-reviews grid. The banner is built the same way as the homepage's diary card and a diary entry's banner, so the three read as one family.
- `week-summary` — the Weekly Overview dashboard's summary, driven by that note's `week-start` property, in the same banner-over-body card: a "Weekly Overview" banner (eyebrow, the week's span as its title, stats, and the week navigator) above the seven-day table. Each table row leads with a logged/empty status dot, splits the weekday from the date, links the entry under its own title with a mood heat dot, and rolls the day's tasks into a count pill; an unlogged day offers a quiet **Add entry** in its place.
...
- `topics-table` — per-topic rollup on a subject index note (how many notes are in each, last activity, open tasks). Scope is the host note's own folder. One **Notes** column, counting whatever note types the journal declares — named after the type where a journal has only one, so a Media journal's shelves read *Films* rather than *Notes*.
...
- `tasks-table[:<folder>]` — the "Open Tasks" rollup: every still-open ChronoAnvil task from notes under `<folder>` (optional, defaults to the host note's own folder), grouped by source note with the note title linked. Tick a checkbox to complete a task in place; edit its text/priority/due in the source note's own Tasks field. Used on the Weekly, Monthly, Staging and Subject dashboards. It also accepts **`./` — the whole vault**, which is the only way to say that from a note that isn't at the top of one, since empty means the *host's* folder; the section editor offers it by name. On the **homepage** the two are the same thing, because the homepage's own folder is the vault root — bare `tasks-table` there means every note you have. Replaces the old Tasks-plugin `` ```tasks `` query blocks.

All of the above read note metadata directly through Obsidian's own cache, so none of them need Dataview.

Legacy syntax — `` `chronoanvil:kind:...` `` written as a single inline code span — still works, and is what the plugin uses for the per-topic buttons inside the homepage's study table (a table cell can't hold a fenced block). Prefer the block syntax everywhere else.
...
- **🏷️ Tags** — the one **global** built-in, and the only one whose value is a
  ...
  reads tags reads these: the tag pane, `tag:` search, and ChronoAnvil's own
  ...
  block — which is where ChronoAnvil puts every section. Typing `#reading` into a
...
touch the region they manage (marked by `# chronoanvil:trackers:start` /
`# chronoanvil:trackers:end` comments in the template, and the columns the plugin
...
A second field, **Carries**, used to sit beside it and is gone. It let a
...
every journal, Study included: the **banner**, the **tracker grid** that sits
...
A note written before the section existed, or one you have added a chart to by hand, keeps
...
A journal's notes live in the vault; its *definition* — the
levels, the kinds, what each is rated on — used to live only in `data.json`, inside
...
there, and ChronoAnvil had no idea they were a journal. It vanished from the
Journals banner and from Settings.

Each journal now keeps its definition **in its own folder**, in a hidden
`.chronoanvil-journal.json` beside its top-level containers. It is written when the
...
notes may well be logging one. Built-in trackers are left out — ChronoAnvil re-seeds
...
A folder that arrives **without** a manifest — an older one, or a
...
ChronoAnvil's own generated files describe the journal precisely.
ChronoAnvil's own generated files describe the journal precisely: every template is
...
ChronoAnvil's settings live in `data.json` inside its own plugin folder, so
...
ChronoAnvil keeps a mirror of its settings at `.chronoanvil-registry.json`
...
settings come back and ChronoAnvil says so.
...
- Renaming or moving the events note in the file explorer retargets the setting automatically, like every other ChronoAnvil path.
...
and a range. ChronoAnvil reads the value straight from each daily note's own
...
much of that grid it takes. By default ChronoAnvil picks one for you from the chart
...
which is the confirmation. Charts only move within the note they are on. The
grid does not **backfill** — a small chart never hops up past a wide one to
fill a hole — because a grid that rearranges itself cannot keep an order you
dragged.
...
`Day-YYYY-MM-DD` filename and filters the window in ChronoAnvil itself, so bounded
...
## How much room a journal note spends before your writing

A journal note opens with chrome — the banner, the grid you log into, the table
of what is below — and on a Lesson with two trackers and no pages yet that used
to be three framed cards and about 440 pixels before the first heading you
actually wrote. Four things bring it down, and all four are things you can
change.

**The banner is the remote control.** Under the note's name it draws **one
chevron for every section welded into its card**, in the order they sit in it,
each labelled with the section's own name — **📊 Trackers**, and whatever the
note calls what is below it (**🗂️ Topics**, **📄 Pages**) — and pressing one
shows or hides that whole section, title and all. They start closed, so a note
you have just opened is a banner, two words and your writing; press a chevron
and the section opens inside the banner's card, directly under the strip, with
its own fold chevron still working. What you choose is remembered per note, so a
Lesson you keep open with its pages showing stays that way and the next Lesson
does not. A section that is not on the note gets no chevron, and a banner with
nothing welded into it draws no strip at all. (The strip reads the fence, so a third
section welded into a banner brings a third chevron with it, called whatever
its own head calls it.)

**The strip only reaches what is inside the banner's card.** A chevron is what a
welded section has instead of a head of its own, so a section sitting in its own
card is not on the strip at all — it has its title bar and its own fold, like
every other section on every other page. That is what breaking a stack up gives
back, and it is why a note whose sections are each in their own card shows no
strip even though both sections are there.

**The grid and the index are drawn inside the banner**, on journal
notes and their indexes — one block holding three sections, which is a **stack**
(see the `stack` directive above; the fence says so on its first line). The grid is where a diary entry's has always been; the table of what is below joined it so that the
chevron above it opens something attached to the banner rather than a card
floating under it. Both are still their own sections — untick **📊 Trackers**
or **🗂️ Topics** in *Edit sections…* and that one goes, the banner stays — but
while they are there the three share one card instead of each spending a head, a
border and a gap of its own.

One consequence is worth knowing: dragging one section of a stack past another
in *Edit sections…* snaps back with *No changes* rather than quietly doing
nothing — reordering inside a stack is not something the writer can do safely, so
it declines out loud. Unticking a section and ticking it again also still gives
it a card of its own, with its own title, below the banner's whole card.

**You can make one yourself.** A note the plugin did not compose one for — one
where you have unticked the grid and ticked it again, say — has the three
sections in three cards until you weld them. Open
*Edit sections…* and the banner's row now carries a **layers** button, outlined
in the stack's own colour so it is not mistaken for the link button beside it —
press it and every section that may be welded goes into the banner's card at
once. Once there is a stack, the same button appears on any of those sections
still sitting outside it, and welds that one in. Which sections may be welded is
the plugin's answer rather than a setting, and it is one list for
every page with a banner rather than a journal note's own: the logging grid and
the table of what is below on a journal note, the search box on Search, and the
Go to grid on the homepage.

A welded section loses the title bar it wears standing alone — the banner is
the head — and gains the banner's chevron; it keeps its own tick, so unticking
it still removes just that section. It stops being able to open a page, because
pages divide a row's columns and a stack has none.

**Any page with a banner can be a stack.** The banner is the host on
every page that has one, and the sections that may be welded into it are:

- **📊 Trackers** and the table of what is below — **🗂️ Topics**, **📄 Pages**
  — on journal notes and their indexes, which is where the arrangement started.
- **The search box** on the Search page, which is the whole reason that page
  exists and the first thing you look for under its name.
- **The Go to grid** on the homepage — the weekly, monthly, quarterly and yearly
  tiles.

The list is the plugin's answer rather than a setting, and it is one list for
every page rather than one per page, so a section that may be welded means the
same thing wherever you meet it. **One thing to know about the homepage:** its
grid ships inside the top row, beside the diary card and your open tasks, and a
section already sharing a block with something else is offered no weld — pulling
it out of an arrangement you made would be one press changing two things. So
take it out of the row first (**Take out of the group**, in the same window) and
the layers button appears on it.

**A journal dashboard's Contents is not in this yet**, and the reason is worth
knowing because it is about how that section titles itself. It has no title bar
of its own — it asks the whole block to be drawn as a section instead, with
`frame: section` — and that line describes a *block*. Welded into the banner it
would describe the banner's block, and the page would lose its card. Giving that
section an ordinary head instead is a small change, but it changes what a
journal dashboard looks like on every vault that has one, so it is a decision
about that page rather than about stacks.

**Diary entries are not in this yet.** An entry's banner already holds its
logging grid, and the machinery it would need — the block editing every other
surface has — is not built for entries, so an entry has no groups, no pages and
no stacks. It is the next surface for this rather than an omission.

**And you can take it apart again.** **Break up the stack** on the
card gives every section in it a block of its own, in the order they were in,
each wearing the title it wears alone — the exact note you would have had if you
had never welded them. They land **directly under the banner**, above whatever
you have written, and each is an ordinary section again: its own head, its own
fold, and no chevron in the banner, which now has nothing left to open. **Take
out of the stack** does the same for one section and leaves the rest welded. The section the stack is built on cannot leave its
own card, because that card is its fence; breaking the stack up is the operation
that means.

What makes both possible is the second half of the `stack` line. A stack's fence
carries one of them **above each section in it** — the first line both declares
the block a stack and opens the section that hosts it — so the file says which
of its lines are whose, exactly as `cell` says where a row's columns divide.
Without that, the only thing that knew where one section ended was the code that
had just composed them, and the window had to refuse: *this stack doesn't say
where each section starts*. **A note composed before the fence said any of this is in that state**, and
the fix is a Save: open *Edit sections…* on it and the plan offers to add the
missing `stack` lines — an *extend*, listed with the note's other repairs — after
which the buttons work. Where a fence has been edited by hand far enough that
the boundaries cannot be worked out, nothing is guessed: the note is left exactly
as it is, still one card, and the button goes on saying why.

**A table with nothing in it says so on its own head.** The Topics table on a
subject and the Pages table on a long note are there before there is anything to
list — they are the way you make the first one — so instead of a box explaining
that, the head keeps its name with a faint *none yet* after it and the **New
lesson** button in its usual place, and no body is drawn at all. As soon as there is one row, the table
appears. Tables that only exist once there is something to show are unchanged,
and so are the empty states in cards you can reach no other way.

**Any section's title bar can come off**, one section at a time, in *Edit
sections…*: every row titled by a bar of its own carries a **Show as widget**
box, and ticking it draws that section bare — no title, no chevron, no fold,
about 44 pixels back. It is the same box the cells of a group carry, and the
answer is written into the note, so it is a property of that note rather than a
setting of the plugin's. A section whose bar holds a control it cannot live
without — the **New topic** button, **+ Add chart** — is not offered the box,
and neither is a section welded into the banner — the grid composes no bar of
...
ChronoAnvil's own pages — the homepage, Search, the dashboards, journal notes and diary entries — open in **reading mode** rather than editing mode. They are pages of widgets, and in editing mode they also show the Properties block. Everything on them still works: the section editor, the cog on the title card, click-to-rename, every button and tracker.

**Ctrl+E still wins** — switching an open note to editing mode is never undone. ChronoAnvil only acts when a note is opened, so switching tabs away and back does open it again.
