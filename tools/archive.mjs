...
//
// UNDER `archives/` RATHER THAN LOOSE BESIDE THE TREE, which is where every zip
// from 5.0.0 on already sits. The two directories were being written at the top
// level and then moved by hand, so the script's idea of where an archive goes
// and the place archives actually are had drifted apart — and a script that
// writes somewhere nobody looks produces a second, stale set rather than an
// error. `mkdir -p` below creates the whole path, so a fresh clone needs no
// setup.
const ARCHIVES = path.resolve(ROOT, "..", "archives");
const BUILDS = path.join(ARCHIVES, "chronoanvil-builds");
const SOURCES = path.join(ARCHIVES, "chronoanvil-source");
...
//
// `coverage` JOINED THE LIST IN 5.2.0, for the same reason `dist` is on it: it is
// a report of one run of one tree, regenerated by `npm run test:coverage` and
