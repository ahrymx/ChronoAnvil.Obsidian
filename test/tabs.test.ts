...
describe("the editor carries the second bit", () => {
  ...
  it("never lets a group's opener begin a page", () => {
    ...
    // A WIDER WINDOW SINCE 5.29, which added the third bit above this line
    // rather than beside it — the page filter is now the second thing
    // `normalise` settles, not the first.
    expect(order.slice(norm, norm + 900)).toContain(
