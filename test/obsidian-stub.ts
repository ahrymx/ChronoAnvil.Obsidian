...
// Which mode an editor is in — Obsidian declares this as a real CodeMirror
// field and so does this, because `ui/fence-cursor.ts` reads it through the
// ordinary `state.field(...)` API and a test that installed something else
// would be testing something else. True here, as Live Preview is the case with
// behaviour; a test asks for the other one with `.init(() => false)`, which is
// CodeMirror's own way to give a field a different starting value.
...
function makeMoment(d: Date, valid: boolean): MomentShim {
  ...
  return {
    ...
    format(fmt: string): string {
      ...
      // `dddd` IS THE FULL NAME AND `ddd` THE SHORT ONE (5.31.2). Both read the
      // short table until this release, which made the stub answer `Sun` where
      // moment answers `Sunday` — and three callers depend on the difference:
      // the daily entry's `titleFormat` (`dddd D MMMM YYYY`), the rollup's
      // caption, and `time-grid-view.ts`, which picks between the two BY NAME
      // depending on how many columns it is drawing. A stub that collapses them
      // cannot fail on that choice being made backwards.
      const DOW_FULL = [
