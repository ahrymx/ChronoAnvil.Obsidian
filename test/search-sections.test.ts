...
describe("the search model", () => {
  ...
  it("does not offer a widget it already writes, including one inside a fence", () => {
    const offered = model.addable(search()).map((s) => s.id);
    // The three this note writes as sections of its own.
    expect(offered).not.toContain("w:diary-search");
    expect(offered).not.toContain("w:on-this-day");
    expect(offered).not.toContain("w:timeline");
    ...
    expect(offered).not.toContain("w:links");
  });
