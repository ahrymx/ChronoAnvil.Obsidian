...
describe("per-entry trackers", () => {
  ...
  describe("journalAncestors", () => {
    ...
    it("returns every folder between the root and the note", () => {
      // WAS "never returns more containers than the levels plus a promotion",
      // and the cap it named has been through three settings. `levels.length`
      // for an unpaged type; `levels.length + 1` from 1.0.23, once every
      // journal could promote a note and the folder a promotion makes became
      // describable; and none at all from 1.0.38, because a page holds pages.
      //
      // THE ARGUMENT THE CAP WAS MAKING — *"a note filed deeper than the type's
      // hierarchy allows shouldn't invent crumbs for folders the type has no
      // noun for"* — is answered by the model rather than by arithmetic now.
      // Every folder below the leaf line is a note that holds the one beneath
      // it, and the noun for all of them is the same one: Page.
      expect(
        journalAncestors(cooking,
...
describe("fold scope in the shipped templates", () => {
  ...
  //
  // FROM THE BAR DOWN, NOT FROM THE FENCE OPEN (5.28). A welded fence has lines
  // ABOVE its bar — the banner and the tracker grid, which the bar does not own
  // — and counting the whole fence would have the pages bar folding the note's
  // own banner away.
  const weldedSiblings = (fenceBlock: string): number => {
    const lines = fenceBlock.split("\n").map((l) => l.trim());
    const bar = lines.findIndex((l) => l.startsWith("header:"));
    return lines
      .slice(bar + 1)
      .filter(
        (l) =>
  ...
  const hiddenAfterFirstBar = (text: string): number => {
    ...
    for (const b of blocksOf(text)) {
      // A BAR NEED NOT OPEN ITS FENCE (5.28). The Lesson's `header:📄 Pages`
      // is welded under the banner now, so a pattern anchored to the fence
      // open reported the note as having no bar at all.
      const isBar = /^```chronoanvil$/m.test(b) && /^header:/m.test(b);
  ...
  it("folds only the pages table on a Lesson, not the whole note", () => {
    ...
    //
    // TWO AS OF 5.28, AND THE HALVING IS THE MEASUREMENT THIS RELEASE WAS FOR.
    // It was four while the grid had a bar of its own — that bar was the FIRST
    // on the page, so the fold scope opened above the grid and hid its two
    // tracker lines as well. The grid is inside the banner now and composes no
    // bar, so the first bar on a Lesson is `header:📄 Pages` and what folding
    // it hides is the pages table welded into it and the skeleton's opening
    // marker. `## Overview` beneath them still terminates the scope, which is
    // the half of the claim that must not change.
    //
    // Going to 1 would mean a marker had been dropped; going up means either
    // the grid grew a bar back or something composed below the skeleton, which
    // is the arrangement rule the catalogue's last entry states.
