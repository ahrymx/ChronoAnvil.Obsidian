...
// ── §N+2. THE JOURNAL INDEX NOTES, WHICH ARE NOT SHIPPED NOTES ────────────
//
// *"I think the implementation got things backwards. […] the type field can be
// removed entirely and reverted to the collapsible sections"*
//
...
//
// IT IS PUTTING BACK WHAT 1.0.16 MERGED. That release consolidated the deepest
// index into one table and migrated the notes into that shape; it never shipped
// (`children-split.ts` says so in full), so this migration exists only until
// the development vaults are put right, and then it is deleted along with the
// grammar it reads.
describe("splitting a journal index's table", () => {
  const src = readSrc("scaffold");
  ...
  it("reports it in migrations, where the rewrites with diffs live", () => {
    ...
    // blurb promises "nothing already in them is touched" and this replaces a
    // create button and a table with a head, a button and a table per note
    // type, so reporting it there would make the promise false in the row that
    // carries it — and reporting it in both would put two rows about one file in
    // front of a reader who reads one diff per file.
    const plan = src.slice(
      src.indexOf("private async planMigrations("),
      src.indexOf("async surveyRepair(")
    );
    expect(plan).toContain("splitChildren(original, type)");
    expect(plan).toContain('ops: [{ kind: "migrate", detail: splitDetail(type) }]');
    // One row per file, with the diff the reader decides on — the same shape
    // every other migration in this group reports.
    expect(plan).toContain("diff: diffText(original, split)");
    ...
    expect(journalsGroup).not.toContain("splitChildren");
  });
  ...
  it("names it in the blurb, because the window is the plan", () => {
    ...
    expect(src).toContain(
      "a journal index's single table split back into one per note type"
    );
  });
  ...
  it("walks the templates with the notes, or it undoes itself", () => {
    ...
    // so leaving it on the merged shape would have the migration undone one
    // folder at a time. `indexSurfaces` carries both, which is also what keeps
    // this door and the kind-add offer looking at one set of files.
    const at = src.indexOf("private async splitChildrenTables(");
    const body = src.slice(at, src.indexOf("\n  }", at));
    ...
    expect(body).toContain("const split = splitChildren(original, type);");
    expect(body).toContain("if (split == null || split === original) continue;");
    ...
    // is what it always was, in both directions.
  ...
  it("is the only door onto it, because `extend` keeps the kind-add promise", () => {
    // `consolidateChildren` HAD TWO DOORS, and the second was
    // `offerDashboardCatchup`: a consolidated card has no per-kind part to be
    // short of, so `extend` had nothing to say when a reader added a note type
    // and `kind-change.ts`'s unhedged promise that *"Dashboards will offer to
    // list the new type"* would have gone unkept. With the parts back that
    // promise is kept by `extend` again, so the reverse rewrite has the repair
    // window and nothing else — one rewrite, one diff, one place to look.
    expect(readSrc("dashboard-catchup")).not.toContain("splitChildren");
    expect(readSrc("dashboard-catchup")).not.toContain("children-split");
  });
});
