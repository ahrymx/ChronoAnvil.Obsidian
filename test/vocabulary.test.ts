...
describe("the docs agree with the UI", () => {
  ...
  //
  // CHANGELOG-ARCHIVE.md WAS IN THE LIST UNTIL 1.0.0, and it is out because
  // the file is out: the pre-release notes are not part of a repository whose
  // first release is 1.0.0. It held ~90% of the words the changelog ever
  // contained, so this sweep is a great deal smaller than it was — which is
  // the honest state, and the reason the existsSync above is not relaxed. A
  // list that skipped a missing file would look exactly the same on the day
  // somebody deletes README.md.
  const READER_FACING = ["README.md", "CHANGELOG.md"];
