...
describe("what the mirror carries", () => {
  ...
  it("leaves out the four things that are a moment, not a configuration", () => {
    // A FOURTH IN 5.28, and it is the same kind of fact as the first: which of
    // a note's sections the reader had OPEN. The list is read rather than
    // restated, so a fifth needs no edit here — what this pins is that every
    // name on it is genuinely withheld.
    const part = mirroredPart(
      settings({
        collapsedNoteSections: { "a.md::Charts": true },
        revealedNoteSections: { "a.md::trackers": true },
