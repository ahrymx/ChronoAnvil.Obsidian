...
//
// WHY A COPY EXISTS AT ALL. Every colour in this plugin is a `--ca-*` custom
// property, read by a stylesheet — and a `.canvas` file is JSON that Obsidian
// draws itself. Our stylesheet never sees it, so `var(--ca-ev-teal)` in a
...
// is `--ca-ev-*`, and this test is what makes that sentence true.
...
// The `--ca-ev-<name>` definitions in the token file, as a name→hex map.
//
// COMMENTS OUT FIRST, on `test/tokens.test.ts`'s own argument: prose in these
// files names tokens, and a sentence beginning `--ca-ev-teal: …` at the start
...
function tokenHues(): Record<string, string> {
  ...
  for (const m of css.matchAll(/^\s*--ca-ev-([a-z]+)\s*:\s*(#[0-9a-fA-F]{3,8})\s*;/gm)) {
...
describe("canvas palette", () => {
  it("matches --ca-ev-* in styles/00-tokens.css, colour for colour", () => {
    ...
    for (const name of EVENT_COLORS) {
      ...
      if (token !== canvas) drift.push(`--ca-ev-${name}: ${token} but CANVAS_HUE.${name}: ${canvas}`);
    }
