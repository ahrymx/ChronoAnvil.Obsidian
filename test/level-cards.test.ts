...
//
// These assertions read `border: 1px solid var(--ca-border-inner)` and broke
...
const hasEdge = (rule: string, side = ""): boolean =>
  rule.includes(`border${side}: 1px solid var(--ca-border-inner)`) ||
  (rule.includes(`border${side}-width: 1px`) &&
    rule.includes(`border${side}-style: solid`) &&
    rule.includes(`border${side}-color: var(--ca-border-inner)`));
...
describe("the layout answers to the block, not the window", () => {
  it("sizes its columns off the container", () => {
    ...
    const rule = cssRule(".ca-jld-grid");
  ...
  it("keeps a pair together as one cell of that grid", () => {
    ...
    const rule = cssRule(".ca-jld-pair");
  ...
  it("stacks the pair under the width two cards need, by container query", () => {
    ...
    const stacked = cssRule(".ca-jld-pair").split("\n");
    ...
    expect(enclosingQuery(".ca-jld-pair")).toContain("max-width: 560px");
  });
  ...
  it("gives the container card the shared stat strip, not a band of its own", () => {
    ...
    expect(cssRule(".ca-jld-card-body .ca-stat-value")).toContain("font-size");
  });
});
...
describe("a pair is one box, not two boxes side by side", () => {
  it("puts the border on the pair and takes it off the cards inside", () => {
    ...
    const pair = cssRule(".ca-jld-pair");
    ...
    const inner = cssRule(".ca-jld-pair > .ca-jld-card");
  ...
  it("draws exactly one divider inside it, and turns it when the pair stacks", () => {
    ...
    // `.ca-jld-pair > .ca-jld-card:first-child` until the pair took a single head: the
    ...
    // `.ca-jld-container` is a name the left pane has carried since 4.36.
    expect(hasEdge(cssRule(".ca-jld-pair > .ca-jld-container"), "-inline-end")).toBe(
      true
    );
    expect(readCss()).not.toContain(".ca-jld-pair > .ca-jld-card:first-child");
    const stacked = cssRule(".ca-jld-pair > .ca-jld-container");
    ...
    expect(enclosingQuery(".ca-jld-pair > .ca-jld-container")).toContain(
      "max-width: 560px"
    );
  });
  ...
  it("carries one head across both panes, spanning the tracks (4.38)", () => {
    ...
    expect(cssRule(".ca-jld-pair > .ca-journal-sec")).toContain("grid-column: 1 / -1");
    ...
    expect(cssRule(".ca-jld-pair > .ca-journal-sec")).not.toContain("span 2");
    ...
    expect(cssRule(".ca-jld-pair > .ca-journal-sec")).toContain("min-height:");
    expect(cssRule(".ca-jld-card > .ca-journal-sec")).toContain("min-height:");
  });
  ...
  it("says what each pane holds in the pane, as a caption and not a title", () => {
    ...
    // look the same and behave differently; caps at `--ca-text-2xs` is the
    // plugin's treatment for a thing that NAMES A REGION and cannot be mistaken
    // for a destination.
    expect(fn("paneLabel")).toContain('cls: "ca-jld-pane-label"');
    expect(fn("paneLabel")).toContain("text.toUpperCase()");
    const label = cssRule(".ca-jld-pane-label");
    expect(label).toContain("var(--ca-caps-weight)");
    expect(label).toContain("var(--ca-caps-tracking)");
    ...
    expect(fn("childrenCard")).toMatch(
      /paneLabel\(card,[\s\S]*?createDiv\(\{ cls: "ca-jjs-card-body" \}\)/
    );
    ...
    expect(cssRule(".ca-jld-pane-label + .ca-jld-card-body")).toContain("padding-top: 2px");
    expect(readCss()).not.toContain(".ca-jld-pane-label + .ca-jjs-card-body");
  });
  ...
  it("has a rule under the head, which 4.36.3 wrongly claimed it had removed", () => {
    ...
    // about. It read:
    //
    //     expect(cssRule(".ca-jld-card > .ca-journal-sec")).not.toContain("border-bottom");
    //
    // and it passed, and the border was there the whole time. 4.36.3 removed the
    // DECLARATION from that rule; `.ca-journal-sec`'s base rule still paints
    // `var(--ca-rule-hair) solid var(--background-modifier-border)` on every bar
    // in the plugin, and nothing cancels it for a card head — the sheet's only
    // `border-bottom: none` rules are scoped to `.ca-journal-header-bar` variants,
    ...
    expect(cssRule(".ca-journal-sec")).toContain("border-bottom: var(--ca-rule-hair)");
    expect(readCss()).not.toContain(".ca-jld-card > .ca-journal-sec.is-");
    const head = cssRule(".ca-jld-card > .ca-journal-sec");
  ...
  it("stops the stat strip drawing hairlines inside a card", () => {
    // `.ca-stats` divides its cells by showing a border-coloured ground through
    // the gaps, which is right for a framed masthead band and is, inside a card,
    // a grid drawn inside a box drawn inside a section.
    const strip = cssRule(".ca-jld-card-body .ca-stats");
    expect(strip).toContain("background: transparent");
    expect(cssRule(".ca-jld-card-body .ca-stat")).toContain("background: transparent");
  });
});
...
describe("the head wears the journal's own hue (4.37)", () => {
  it("paints from --jjc-hue, set on the grid by the builder", () => {
    ...
    expect(cssRule(".ca-jld-card > .ca-journal-sec")).toContain("var(--jjc-hue");
  });
  ...
  it("mixes into a theme surface rather than stating a literal colour", () => {
    ...
    // below the `:root` close, and the reason `--ca-border-inner` needs no
    ...
    expect(cssRule(".ca-jld-card > .ca-journal-sec")).not.toMatch(/#[0-9a-f]{3,6}/i);
  });

  it("is the same material as the journals card's banner, not a second colour", () => {
    // `.ca-jjc-banner`'s own base is `hsl(var(--jjc-hue) 45% 42%)`, and the head
    ...
    expect(cssRule(".ca-jld-card > .ca-journal-sec")).toContain("hsl(var(--jjc-hue, 260) 45% 42%)");
    expect(cssRule(".ca-jjc-banner")).toContain("hsl(var(--jjc-hue, 260) 45% 42%)");
  });
  ...
  it("states its own height so the pair's band has no step in it (4.38)", () => {
    ...
    // `.ca-journal-header-title`'s line-height and `--ca-sec-title-size` its size, so
    ...
    expect(head).toContain("var(--ca-sec-title-size)");
    expect(head).not.toMatch(/min-height:\s*\d+px/);
    // AND THE PADDING IS INSIDE THE FLOOR. Obsidian sets box-sizing per component
    // rather than globally — `.ca-jjs-card-body` states this and `.ca-jjh-stat` records
    ...
    const add = cssRule(".ca-jld-head-add");
  ...
  it("lets the band grow on a phone, on both heads at once", () => {
    // THE TAP FLOOR IS TALLER THAN THE TITLE. `--ca-control-min` is 40px under
    // `body.is-mobile` and applies to `.ca-journal-btn-ghost`, so on a phone the ＋
    ...
    const mobile = cssRule("body.is-mobile .ca-jld-card > .ca-journal-sec");
    expect(mobile).toContain("var(--ca-control-min)");
    // Not one rule with a `max()`: `--ca-control-min` is the string `auto` on the
    // desktop, and `auto` is not a length a `max()` can take.
    expect(cssRule(":root")).toContain("--ca-control-min: auto");
  });
...
describe("the add tile is the size of the slot it opens (4.37)", () => {
  it("spans a pair's footprint where the grid draws pairs", () => {
    ...
    //
    // `span 2` IS `.ca-jld-pair`'s OWN DECLARATION, which is what makes them line up
    // rather than merely both being wide.
    expect(cssRule(".ca-jld-grid.is-paired .ca-jld-add-tile")).toContain("grid-column: span 2");
    expect(cssRule(".ca-jld-pair")).toContain("grid-column: span 2");
  });
  ...
  it("decides from the journal's shape, not from a width", () => {
    ...
    expect(enclosingQuery(".ca-jld-grid.is-paired .ca-jld-add-tile")).toBe("");
  });
});
...
describe("the strip collapses against the card it is in", () => {
  it("makes the card a query container", () => {
    ...
    // container, which was `.ca-journal-widget-block` — the whole section, never
    ...
    // that the card.
    expect(cssRule(".ca-jld-card")).toContain("container-type: inline-size");
    ...
    expect(cssRule('.ca-stats[data-cols="4"]')).toContain("grid-template-columns");
    expect(readCss()).not.toContain(".ca-journal-widget-block .ca-stats[data-cols");
  });
  ...
  it("collapses four cells earlier in a card than the shared rule does", () => {
    ...
    expect(
      enclosingQuery('.ca-jld-card-body .ca-stats[data-cols="4"]')
...
describe("the create control belongs to the surface, not to the card", () => {
  ...
  it("puts the list's control in a head and nothing in its body (4.37)", () => {
    ...
    // row, and it cost one of the four rows `.ca-jjs-card-body` allows — a card
  ...
  it("gives the head control a label it draws and hides, not a bare glyph", () => {
    ...
    expect(body).toContain('cls: "ca-journal-btn-ghost ca-jld-head-add"');
    expect(body).toContain('cls: "ca-journal-btn-label"');
    ...
    const shown = cssRule(
      ".ca-jld-card:hover > .ca-journal-sec .ca-jld-head-add .ca-journal-btn-label"
    );
    ...
    const rest = cssRule(".ca-jld-head-add .ca-journal-btn-label");
    ...
    expect(enclosingMedia(".ca-jld-head-add .ca-journal-btn-label")).toContain("hover: none");
  });

  it("states both ＋ sizes rather than inheriting them (4.38)", () => {
    // THE HEAD ＋ WAS 12.24px AND NOTHING SAID SO: `.ca-journal-btn-ghost` is
    // `--ca-text-sm` (0.85em → 13.6px) and its icon rule is `0.9em` of that. Two
    ...
    const head = cssRule(".ca-jld-head-add.ca-journal-btn-ghost .ca-journal-btn-icon");
    ...
    // `.ca-journal-btn-ghost .ca-journal-btn-icon` in 85-tracker-controls.css is two
    // classes, and 85 sorts AFTER 60 — so at equal specificity the shared rule wins.
    // Mutation-checked by dropping `.ca-journal-btn-ghost` from the selector.
    const css = readCss();
    expect(css.indexOf(".ca-jld-head-add.ca-journal-btn-ghost .ca-journal-btn-icon")).toBeLessThan(
      css.indexOf(".ca-journal-btn-ghost .ca-journal-btn-icon {")
    );
    ...
    const tile = cssRule(".ca-jld-add-icon");
    expect(tile).toContain("--jld-add-glyph: 20px");
    expect(tile).toContain("width: var(--jld-add-glyph)");
    expect(cssRule(".ca-jld-add-icon svg")).toContain("width: 100%");
  });
  ...
  it("reads as an empty slot, in the vocabulary a tracker already uses", () => {
    ...
    const tracker = cssRule(
      ".ca-journal-tracker-bar .ca-journal-tracker-cell.ca-journal-tracker-add"
    );
  ...
  it("offers the tile in the empty state and stops citing a control elsewhere", () => {
    ...
    expect(cssRule(".ca-jld-grid.is-empty")).toContain("grid-template-columns");
  });
