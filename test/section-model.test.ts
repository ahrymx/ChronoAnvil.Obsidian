...
describe("which sections may weld into a banner", () => {
  // ONE LIST FOR TEN CATALOGUES (5.30). `JournalSection.stack` declared this on
  // three sections of one catalogue, and `sections.ts` said in as many words
  // what would happen the day a second catalogue wanted it: *"the field moves up
  // beside `repeatable` and this line goes."* The reader asked for that day —
  // *"I think the stack could become a global thing for any page with a
  // banner"* — and chose the shape after being told the trade: *"the
  // eligibility list is global, for now it is only trackers and what's below."*
  //
  // A GLOBAL LIST IS ONLY WORTH MORE THAN TEN DECLARATIONS IF SOMETHING CHECKS
  // IT AGAINST ALL TEN, which is what these three do. `goldenNotes` is the
  // enumeration of every composed note and the model that edits it, so a new
  // catalogue joins this sweep by existing rather than by being added to it.
  const catalogues = (): { name: string; model: SectionModel; text: string }[] =>
    goldenNotes().map((n) => ({ name: n.name, model: n.model(), text: n.text }));

  it("names only ids some catalogue actually offers", () => {
    // A word in this list that no catalogue has is a weld nothing can perform,
    // and it reads exactly like one that works. `contents`, `search` and
    // `launcher` were added from three different files in one release; this is
    // what would have caught a typo in any of them.
    const offered = new Set<string>();
    for (const { model } of catalogues())
      for (const v of model.sections()) offered.add(v.id);
    for (const id of WELDS_INTO_BANNER) expect([...offered], id).toContain(id);
  });

  it("is the only thing any catalogue offers a weld from", () => {
    // THE OTHER DIRECTION, and the one that keeps the list the single answer. A
    // `stacks` host set from anywhere but this list would be a second
    // declaration of the same fact, which is the shape the field had.
    for (const { name, model } of catalogues()) {
      for (const v of model.sections()) {
        if (v.stacks === undefined) continue;
        expect(v.stacks, `${name}/${v.id}`).toBe(BANNER_ID);
        expect([...WELDS_INTO_BANNER], `${name}/${v.id}`).toContain(v.id);
      }
    }
  });

  it("offers one only where the surface has a banner and can arrange blocks", () => {
    // THREE THINGS A WELD NEEDS, and a surface missing any of them offers a
    ...
    for (const { name, model } of catalogues()) {
      ...
      if (!views.some((v) => v.stacks !== undefined)) continue;
      welding++;
      expect(views.map((v) => v.id), name).toContain(BANNER_ID);
      expect(typeof model.blocks, `${name} blocks`).toBe("function");
      expect(typeof model.regroup, `${name} regroup`).toBe("function");
    }
    // And it is not vacuous: the sweep runs against surfaces that offer one.
    expect(welding).toBeGreaterThan(1);
  });
});
...
describe("the editor cannot learn which surface it is on", () => {
    ...
    for (const { name, model } of surfaces()) {
      for (const s of model.sections()) {
        expect(Object.keys(s).sort(), `${name}/${s.id}`).toEqual([
          ...
          "removable",
          // 5.29, and the third field this assertion made somebody argue for.
          //
          // THE READER ASKED FOR THE GESTURE: *"the section editor needs a way
          // for users to initiate the banner as a stack … then approved widgets
          // (for now that is trackers and what's below) can be linked into the
          // stack."* Approved by WHOM is the whole question §2 asks. The two
          // answers available were a list in the window — which is the editor
          // knowing `trackers` and `children` by name, and therefore knowing it
          // is on a journal note — and this field, which is the catalogue's own
          // `JournalSection.stack` said to the editor. It is `category`'s
          // argument one release on: the window draws a button where a section
          // says it may be welded, and learns nothing about which sections
          // those are.
          //
          // ONLY WHERE THERE IS AN ANSWER, on `questions`' rule and for its
          // reason — present-and-undefined on every row would make the key
          // universal and defeat this assertion.
          ...(s.stacks ? ["stacks"] : []),
        ].sort());
      }
    }
  ...
  it("and every section that asks is one somebody decided should", () => {
    ...
    // no bar at all.
    //
    // `trackers` WAS THE ONE THE RULE WAS RESTATED FOR, AND IS OFF THE LIST
    // AGAIN AS OF 5.28. It was out while the rule also refused a fence owning a
    // managed region, and came back because the reader's rule is the action row
    // and nothing else — the grid's "+ Add tracker" is a TILE IN THE GRID
    // rather than a button in the bar, so it travelled with the widget wherever
    // the widget went. What ends it is not that rule but the arrangement: on
    // both journal surfaces the grid is now welded into the banner's fence, and
    // `widgetFormBar` refuses a welded section outright. It composes no bar of
    // its own, so there is nothing for the toggle to take off. Seven qualify
    // where eight did, and the same derivation answers for the difference.
    //
    ...
    // THE ARGUMENT — and one of the two has since left for good, which is the
    // reason to keep reading. The journal index this test builds is the DEEPEST
    // one, where until 5.28 the tracker grid and the stats band were the two
    // pages of one group, so neither composed a bar. 5.18 read that as "no bar
    // to report, no toggle
    ...
    //
    // AND 5.28 DISSOLVED THE GROUP THE PARAGRAPH ABOVE IS ABOUT. The grid went
    // into the banner and the band got its own card and its own bar back, so
    // `journal index/stats` is on this list for the ordinary reason now and
    // `journal index/trackers` is on it for none. The 5.21 rule is untouched:
    // it is still what answers for a cell whose bar arrives by a gesture, and
    // `review`/`tasks` is still such a pair.
    //
