...
import { MODIFIER_KEYWORDS } from "../src/core/directive-grammar";
...
describe("the section catalogue", () => {
  ...
  describe("stays in step with the shipped Study templates", () => {
    ...
    // A MODIFIER IS NOT A WIDGET AND NEVER WILL BE CLAIMED. `header` was listed
    // here by name; `stack` joined it in 5.28 and would have been the third
    // release to add one by hand, so the set is asked of the grammar instead —
    // `MODIFIER_KEYWORDS` is the list of lines that say something ABOUT a block
    // rather than being one, which is exactly the exemption this needs.
    const CHROME = new Set(["spacer", "button", ...MODIFIER_KEYWORDS]);
