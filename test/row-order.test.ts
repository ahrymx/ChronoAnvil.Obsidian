...
describe("the bits stay describable", () => {
  it("drops a bit on a row with nothing above it", () => {
    ...
    const out = normalise(["a", "b"], new Set(["a"]), new Set(), new Set());
    expect([...out.joined]).toEqual([]);
  });

  it("holds a page break to a row that is in a group", () => {
    const out = normalise(["a", "b"], new Set(), new Set(["b"]), new Set());
    expect([...out.paged]).toEqual([]);
  });
...
describe("the window is thin over it", () => {
  ...
  it("keeps no reordering of its own", () => {
    ...
    for (const write of [
      ...
      "this.paged.add(",
      "this.paged.delete(",
      // AND THE THIRD BIT THE SAME WAY (5.29). It is written by the reader now
      // — *make this a stack* — so it is exactly the kind of thing that would
      // otherwise be toggled in place beside a button, with the block bits
      // patched up next to it.
      "this.stacked.add(",
      "this.stacked.delete(",
    ]) {
  ...
  it("takes every change through one place", () => {
    // `settle` is the only writer of the four fields, so a control cannot be
    // added that changes two of them and forgets the rest — which is what the
    // weld would have been, had it been a bit set beside a button.
    const src = editor();
