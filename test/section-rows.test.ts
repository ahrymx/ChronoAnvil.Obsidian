...
describe("the window over it", () => {
  ...
  it("offers no split it could not make", () => {
    ...
    // THE LABEL NAMES THE ARRANGEMENT AS OF 5.29 — "the group" or "the stack",
    // read off the same bit the card behind the row reads — so the anchor is
    // the half of it that does not change.
    const at = editor.indexOf('"aria-label": `Take out of the ${noun}`');
  ...
  it("keeps the split with the arrows and away from Remove", () => {
    ...
    const out = editor.indexOf('"aria-label": `Take out of the ${noun}`');
    ...
    expect(editor).not.toContain('text: `Take out of the ${noun}`');
  });
  ...
  it("lets a locked section leave a row", () => {
    ...
    const block = editor.indexOf('"aria-label": `Take out of the ${noun}`');
  ...
  it("previews the regroup by running it, not by diffing intentions", () => {
    ...
    // FOUR ARGUMENTS SINCE 5.29 AND SO NO LONGER ONE LINE: the blocks, the
    // page boundaries and the welds are three answers about one arrangement,
    // and all three are read from the rows the Save will write.
    expect(body).toContain("this.model.regroup?.(");
    expect(body).toContain("this.welds(idsOf(this.want))");
