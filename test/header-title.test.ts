...
describe("the defect that shipped", () => {
  ...
  it("argSpanIn reads the FIRST header, whichever section asked", () => {
    ...
    // The FIRST header on the page, which is the stats band's — and it belongs
    // to neither of the two sections above it in catalogue order. The banner
    // composes no bar at all, and the tracker grid is welded into the banner's
    // fence as of 5.28 and composes none either. (Before that the same line
    // came out `🗂️ What's below`, because the grid and the band were a state
    // row that titled itself from its cells and wrote no `header:` between
    // them; the grid's departure ended the row and gave the band its own bar
    // back.) Which section owns a header is exactly what this function cannot
    // tell you, so the string is the claim rather than a break in it.
    expect(readArg(lines, span!)).toBe("🔢 Stats");
  });
