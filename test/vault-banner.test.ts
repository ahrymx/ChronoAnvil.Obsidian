...
describe("what the Banner section became", () => {
  ...
  it("has an answer on every surface the bar draws on", () => {
    ...
    // AND THE TWO FLAT NOTES SAY WHAT DAY IT IS (5.31.2). They said nothing for
    // five releases, on an argument that was right about what it turned down —
    // *an eyebrow reading HOME over a title reading Homepage is the doubling
    // this release removes* — and could not find a second fact. The date is it:
    // `home-sections.ts` calls the homepage *"the only note in the vault that
    // is about NOW"*, and nothing in the bar carries a date on any page.
    expect(t).toContain('return moment().format("dddd · D MMMM YYYY");');
    expect(t).not.toContain("HOME AND SEARCH GET NONE, DELIBERATELY");
    // FROM `core/util`, NOT FROM `obsidian`. Obsidian types its own export as a
    // namespace, which does not typecheck as callable; `util.ts` holds the one
    // cast. `eslint`'s `no-restricted-globals` catches the bare global and has
    // nothing to say about which module a named import came from, so it is
    // asserted here.
    expect(t).toContain("moment,");
    expect(t).not.toContain('moment } from "obsidian"');
  });
  ...
  it("takes the page's ground rather than another card", () => {
    ...
    // ── AND THIS TEST WAS READING THE WRONG RULE (5.31.2) ────────────
    //
    // `indexOf(".ca-journal-page-head {")` finds the first occurrence in the
    // whole concatenated sheet, and `30-header-bars.css` sorts before
    // `98-page-head.css` while holding `.ca-journal-stack .ca-journal-page-head
    // {` — which CONTAINS that string. So this has been asserting the stacked
    // head's rule since 5.28, and passed 5.31.2's wash and spine landing on the
    // head without noticing either. Anchored to a line start now, which is the
    // guard `ruleFor` in page-head.test.ts was written around after the same
    // class of miss.
    const css = readCss().replace(/\/\*[\s\S]*?\*\//g, "");
    const at = css.search(/(^|\n)\.ca-journal-page-head\s*\{/);
    expect(at).toBeGreaterThan(0);
    const rule = css.slice(css.indexOf("{", at), css.indexOf("}", at));
    // NO CARD still means no corner and no elevation. It no longer means no
    // background: the wash is a gradient off the spine, and the page's own
