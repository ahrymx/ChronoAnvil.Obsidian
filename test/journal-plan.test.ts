...
describe("reading a template back", () => {
  ...
  it("leaves a widget-form section's missing bar alone (5.11)", () => {
    ...
    //
    // AND OVER THE RICH INDEXES, WHICH IS NOT BELT AND BRACES AS OF 5.28. The
    // shipped templates carry three sections and the tracker grid was the only
    // one of them with a toggle; welding the grid into the banner took its bar
    // away and this loop went to zero passes — the vacuous-sweep failure the
    // last assertion in this test exists to catch, arriving for the third time
    // in this file. The rich fixtures carry `find`, `progress` and `resources`,
    // all of which have one.
    let checked = 0;
    for (const t of [...allTemplates(), richIndex(0), richIndex(1)]) {
...
describe("reordering", () => {
  ...
  it("restores the file exactly when a swap is undone", () => {
    ...
    const present = sectionsPresent(text, ctx);
    const want = [...present];
    // NOT THE FIRST TWO, AS OF 5.28. `banner` and `trackers` are one fence and
    // therefore one chunk, and a swap that names a member of a shared block is
    // the move 4.44.1 refuses — `applySections` returns null and there is
    // nothing to undo. The property under test is about a swap the planner
    // makes, so it is asked of two sections that have a block each.
    const a = present.indexOf("children");
    const b = present.indexOf("path");
  ...
  it("refuses to move the tracker grid out of the banner's block (5.28)", () => {
    // THE COST OF THE WELD, STATED. The two share a fence, so the chunk the
    // reorder permutes holds both and neither can be dragged past the other —
    // the same rule that has governed a row's cells since 4.44.1, arriving on a
    // pair the catalogue welds rather than one a reader grouped.
    //
    // NOT SILENT. The section editor plans by RUNNING the write and reading its
    // result (`SectionEditorModal.readCaps` and the changes pane), so a reader
    // who drags the grid down sees the list snap back and "No changes" rather
    // than a save that quietly does nothing.
    //
    // AND THERE IS A WAY OUT: untick Trackers, save, tick it again. The add
    // path composes the section on its own and `bar: TRACKERS_BAR` comes back
    // with it — asserted below.
    const { ctx, text } = topic();
    const present = sectionsPresent(text, ctx);
    expect(present.slice(0, 2)).toEqual(["banner", "trackers"]);
    const want = [...present];
    [want[1], want[2]] = [want[2], want[1]];
    expect(applySections(text, ctx, want)).toBeNull();
  });

  it("gives the grid its own card back when it is removed and re-added (5.28)", () => {
    const { ctx, text } = topic();
    const present = sectionsPresent(text, ctx);
    const without = applySections(
      text,
      ctx,
      present.filter((id) => id !== "trackers")
    )!;
    expect(without).not.toContain("# chronoanvil:trackers:start");
    const back = applySections(without, ctx, present)!;
    expect(back).toContain(
      ["```chronoanvil", "header:📊 Trackers", "# chronoanvil:trackers:start"].join("\n")
    );
    // BELOW THE BANNER'S BLOCK, NOT BACK INSIDE IT — and below the index that
    // is welded in beside it, because the block is one chunk and a chunk
    // cannot be split. That is the honest answer rather than the tidy one: the
    // grid's rank falls between `banner` and `children`, there is no slot
    // there, and `insertionPoint` puts it after the chunk rather than above it.
    const backOrder = sectionsPresent(back, ctx);
    expect(backOrder).toEqual(
      present.filter((id) => id !== "trackers").toSpliced(2, 0, "trackers")
    );
    expect(backOrder.indexOf("trackers")).toBeGreaterThan(backOrder.indexOf("banner"));
    expect(backOrder.indexOf("trackers")).toBeGreaterThan(backOrder.indexOf("children"));
    expect(new Set(backOrder)).toEqual(new Set(present));
  });
