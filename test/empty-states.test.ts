...
import { allSrcNames, readCode, readCss, readSrc } from "./sources";
import { EMPTY_HEAD_ATTR, emptyHead, liftEmptyHead } from "../src/ui/empty";
...
describe("one home for the shapes", () => {
  ...
  it("distinguishes the three shapes structurally, not by taste", () => {
    // One replaces content, one annotates content that drew itself. A callout
    // inside a card that already has a header is a box inside a box. The third
    // draws no body at all and hands its phrase to the section's own bar.
    const t = src("empty.ts");
    expect(t).toContain("REPLACES content");
    expect(t).toContain("ANNOTATES content");
    expect(t).toContain("THE HEAD CARRIES IT");
  });

  it("keeps tables.ts re-exporting, so a dozen importers stay put", () => {
    const t = src("tables.ts");
    expect(t).toContain('from "./empty"');
    expect(t).toContain("export { emptyCallout, emptyHead, emptyLine };");
...
// ── THE THIRD SHAPE, EXERCISED (5.28) ─────────────────────────────────────
//
// The suite runs in node with no DOM, so everything DOM-shaped in this tree is
// asserted structurally. `liftEmptyHead` is the exception worth paying for: it
// is a THREE-BRANCH DECISION — no marker, a bar with no title, a titled bar —
// and the two branches that must do nothing are the ones that would be silently
// wrong. A source-text sweep cannot tell those apart.
//
// SO THE DOUBLE IS HAND-ROLLED rather than jsdom. What the function touches is
// six methods and one global, and a devDependency plus a test environment is a
// large thing to add for six methods. The double is deliberately literal: it
// records calls rather than emulating a tree, so a test that passes cannot be
...
class FakeEl {
  ...
  constructor(public title = false) {}
  ...
  // The one thing a live host wraps, which is where the marker sits for every
  // table this is written for — see `liftEmptyHead`.
  ...
  querySelector(sel: string): FakeEl | null {
    if (sel === ".ca-journal-header-title") return this.title ? new FakeEl() : null;
    // The phrase a previous pass put on this bar, which the next one takes off
    // before deciding whether to write it again.
    return this.children.find((c) => c.getAttribute(EMPTY_HEAD_ATTR) !== null) ?? null;
  }
  insertAdjacentElement(where: string, el: FakeEl): void {
    if (where !== "beforebegin") return;
    this.before.push(el);
    // Into the bar too, before the actions, which is where it really lands and
    // what lets the next pass find it again.
    el.parentElement = this.parentElement;
    this.parentElement?.children.splice(
      Math.max(0, this.parentElement.children.indexOf(this)),
      0,
      el
    );
  }
...
// A bar and its action slot, as `sectionFrame` builds them: the slot is the
// LAST child of the root, which is why the lift inserts before it rather than
// appending.
const barWith = (title: boolean): { bar: FakeEl; actions: FakeEl } => {
  const bar = new FakeEl(title);
  const actions = new FakeEl();
  actions.parentElement = bar;
  bar.children.push(actions);
  return { bar, actions };
};

// A live-scoped widget as the dispatcher receives it: the `.ca-journal-live-widget`
// host, with the table it wraps inside. `level-index`, `kind-table` and
// `pages-table` all arrive in this shape and none of them arrives bare.
const liveHost = (inner: FakeEl): FakeEl => {
  const host = new FakeEl();
  inner.parentElement = host;
  host.children.push(inner);
  return host;
};

describe("an empty index hands its sentence to the head", () => {
  const spans: { cls: string; text: string }[] = [];
  beforeEach(() => {
    spans.length = 0;
    (globalThis as Record<string, unknown>).createSpan = (o: {
      cls: string;
      text: string;
    }): FakeEl => {
      spans.push(o);
      const el = new FakeEl();
      el.setAttr("cls", o.cls);
      return el;
    };
  });

  it("marks the widget and leaves it otherwise alone", () => {
    const w = new FakeEl();
    expect(emptyHead(w as unknown as HTMLElement, "none yet")).toBe(w);
    expect(w.getAttribute(EMPTY_HEAD_ATTR)).toBe("none yet");
    // The callout the widget already built is untouched — the marker is a
    // permission, not a replacement.
    expect(w.emptied).toBe(false);
  });

  it("moves the phrase onto a titled bar and empties the body", () => {
    const { bar, actions } = barWith(true);
    const w = emptyHead(new FakeEl() as unknown as HTMLElement, "none yet");
    expect(
      liftEmptyHead(w, actions as unknown as HTMLElement)
    ).toBe(true);
    expect(spans).toEqual([{ cls: "ca-journal-header-note", text: "none yet" }]);
    // BEFORE the actions, so it lands beside the title rather than past the
    // button — the same position `sectionFrame` gives a `note` handed in up
    // front.
    expect(actions.before.length).toBe(1);
    expect((w as unknown as FakeEl).emptied).toBe(true);
    expect((w as unknown as FakeEl).classes.has("is-empty")).toBe(true);
    // Consumed, so a second pass over the same element cannot double it.
    expect((w as unknown as FakeEl).getAttribute(EMPTY_HEAD_ATTR)).toBe(null);
    expect(bar.classes.size).toBe(0);
  });

  it("leaves an untitled bar's widget exactly as it drew itself", () => {
    // `untitled: true` is a control strip under a markdown heading. A phrase
    // where no title is drawn is a stray word floating above the button.
    const { actions } = barWith(false);
    const w = emptyHead(new FakeEl() as unknown as HTMLElement, "none yet");
    expect(liftEmptyHead(w, actions as unknown as HTMLElement)).toBe(false);
    expect((w as unknown as FakeEl).emptied).toBe(false);
    expect(spans).toEqual([]);
  });

  it("leaves a widget with no bar above it exactly as it drew itself", () => {
    // The `pages-table` a reader answered `widget` composes with no `header:`
    // line at all, and there the callout is the whole of the card.
    const w = emptyHead(new FakeEl() as unknown as HTMLElement, "none yet");
    expect(liftEmptyHead(w, null)).toBe(false);
    expect((w as unknown as FakeEl).emptied).toBe(false);
    expect(spans).toEqual([]);
  });

  it("reads the marker through the live host that wraps every one of them", () => {
    // THE DEFECT THAT MADE THE WHOLE MECHANISM INERT. `level-index`,
    // `kind-table` and `pages-table` reach the dispatcher through
    // `liveScopedWidget`, which returns a host and appends the built table
    // inside it — so the element being asked never carried the marker and the
    // answer was always no. The next screenshot showed an empty Pages card at
    // full height with the lift "shipped".
    const { actions } = barWith(true);
    const table = emptyHead(new FakeEl() as unknown as HTMLElement, "none yet");
    const host = liveHost(table as unknown as FakeEl);
    expect(
      liftEmptyHead(host as unknown as HTMLElement, actions as unknown as HTMLElement)
    ).toBe(true);
    expect(spans).toEqual([{ cls: "ca-journal-header-note", text: "none yet" }]);
    // THE HOST is what is emptied and marked, because the host is what the
    // dispatcher placed and what `.is-empty:empty` has to take out of the flow.
    expect(host.emptied).toBe(true);
    expect(host.classes.has("is-empty")).toBe(true);
  });

  it("finds the marker however deep the only thing the widget drew is", () => {
    // `level-index` wraps the rollup in a root of its own INSIDE the live host,
    // so the marker is two levels down — and the first cut of this looked one
    // level and no further, which is why the next screenshot still showed a
    // subject's Topics card at full height.
    const { actions } = barWith(true);
    const table = emptyHead(new FakeEl() as unknown as HTMLElement, "none yet");
    const host = liveHost(liveHost(table as unknown as FakeEl));
    expect(
      liftEmptyHead(host as unknown as HTMLElement, actions as unknown as HTMLElement)
    ).toBe(true);
    expect(host.emptied).toBe(true);
  ...
  it("stops at a widget that drew more than one thing", () => {
    // The deepest branch of `level-index` is a head and a table PER KIND. One
    // of those tables being empty is not the section being empty, and lifting
    // its phrase would empty the card on behalf of a fragment.
    const { actions } = barWith(true);
    const host = new FakeEl();
    const first = emptyHead(new FakeEl() as unknown as HTMLElement, "none yet");
    host.children.push(first as unknown as FakeEl, new FakeEl());
    expect(
      liftEmptyHead(host as unknown as HTMLElement, actions as unknown as HTMLElement)
    ).toBe(false);
    expect(host.emptied).toBe(false);
    expect(spans).toEqual([]);
  });

  it("takes the phrase off again when the widget stops being empty", () => {
    // A live widget rebuilds when its folder changes, so adding the first page
    // draws a table with a row in it — under a bar still saying "none yet" if
    // this were a one-shot. Two answers to one question, and the bar's is the
    // one read first.
    const { bar, actions } = barWith(true);
    const host = liveHost(emptyHead(new FakeEl() as unknown as HTMLElement, "none yet") as unknown as FakeEl);
    expect(liftEmptyHead(host as unknown as HTMLElement, actions as unknown as HTMLElement)).toBe(true);
    const note = bar.children.find((c) => c.getAttribute(EMPTY_HEAD_ATTR) !== null);
    expect(note, "the phrase is on the bar").toBeTruthy();

    // The rebuild: a fresh, unmarked table in the host.
    host.children = [new FakeEl()];
    host.classes.add("is-empty");
    expect(liftEmptyHead(host as unknown as HTMLElement, actions as unknown as HTMLElement)).toBe(false);
    expect(note!.removed).toBe(true);
    expect(bar.children.some((c) => c.getAttribute(EMPTY_HEAD_ATTR) !== null)).toBe(false);
    expect(host.classes.has("is-empty")).toBe(false);
  });

  it("never removes a phrase the section itself composed", () => {
    // `sectionFrame` takes a `note:` of its own and it wears the same class.
    // The lift removes only the span it marked, which is why it marks it.
    const { bar, actions } = barWith(true);
    const composed = new FakeEl();
    bar.children.unshift(composed);
    const host = liveHost(new FakeEl());
    expect(liftEmptyHead(host as unknown as HTMLElement, actions as unknown as HTMLElement)).toBe(false);
    expect(composed.removed).toBe(false);
    expect(bar.children).toContain(composed);
  });

  it("is re-run on every redraw, from the element the redraw happens on", () => {
    // The sync above is only as good as its second call. `LiveWidget` fires one
    // event on its host after each build and the dispatcher listens on the
    // widget it just placed — an element listener, so it dies with the node and
    // the `document`/`window` sweep has nothing to say about it.
    const live = readCode("livewidget");
    expect(live).toContain('export const LIVE_REDRAW_EVENT = "ca-live-redraw";');
    expect(live).toContain(
      "this.containerEl.dispatchEvent(new CustomEvent(LIVE_REDRAW_EVENT));"
    );
    const t = readCode("widgets");
    const at = t.indexOf("liftEmptyHead(widget, headerGroup)");
    expect(at).toBeGreaterThan(-1);
    expect(t.slice(at, at + 600)).toContain(
      "widget.addEventListener(LIVE_REDRAW_EVENT"
    );
  });

  it("ignores a widget that never asked, marker absent", () => {
    const { actions } = barWith(true);
    const w = new FakeEl();
    expect(
      liftEmptyHead(w as unknown as HTMLElement, actions as unknown as HTMLElement)
    ).toBe(false);
    expect(w.emptied).toBe(false);
    expect(spans).toEqual([]);
  });

  it("is what the three index tables reach for, and only those three", () => {
    // The rule is narrow and the reason is the button: these are the three
    // sections composed as `header:` + `button:` + table, so the callout's
    // second half — how to make it happen — is already on the bar. The tag
    // index and the task tables have no create button, so their callouts stay.
    //
    // `buildPagesTable` WAS THE MISSING THIRD, and it was the worst of them:
    // a fresh lesson's Pages card is nothing but its empty state, which is the
    // 185px the screenshots caught. It was missed because this assertion said
    // "two" and named the two it had.
    const t = readCode("tables.ts");
    expect([...t.matchAll(/emptyHead\(root, /g)].length).toBe(3);
    for (const fn of ["folderRollup(", "kindTable(", "buildPagesTable("]) {
      const at = t.indexOf(`export function ${fn}`);
      expect(at, fn).toBeGreaterThan(-1);
      expect(
        t.slice(at, t.indexOf("\nexport function", at + 10)),
        fn
      ).toContain("emptyHead(root, ");
    }
  });

  it("is lifted where the dispatcher still knows which bar it is under", () => {
    // Before the appends and before the branch that nulls `headerGroup` for a
    // block widget — after either, the bar this widget landed under is gone or
    // the element has already moved.
    const t = readCode("widgets");
    const build = t.indexOf("const widget = this.buildFromSpec(");
    const lift = t.indexOf("liftEmptyHead(widget, headerGroup)");
    expect(build).toBeGreaterThan(-1);
    expect(lift).toBeGreaterThan(build);
    expect(lift).toBeLessThan(t.indexOf("container.appendChild(widget)", build));
  });

  it("takes the emptied element out of the flow rather than the DOM", () => {
    // `drawn` records a widget's index among its container's children BEFORE
    // the append, so a directive that contributed no child hands its line to
    // the next one and `stampLines` mis-stamps every widget after it.
    const css = readCss();
    expect(css).toContain(".ca-journal-table.is-empty:empty");
    expect(readSrc("empty.ts")).toContain("EMPTIED, NOT DROPPED");
  });
});
