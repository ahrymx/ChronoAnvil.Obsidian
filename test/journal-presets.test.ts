...
import { STACK_KEYWORD } from "../src/core/directive-grammar";
...
//
//   AND THE GRID IS INSIDE THE BANNER AS OF 5.28, so it opens no block of its
//   own and appears in none of the rows below — the Trackers bar every one of
//   them carried until this release. It is still its own section, still its own
//   tick, still draggable out; what it no longer has is a card of its own. See
//   `JournalSection.weld`.
//
//   THAT MAKES THE MEASUREMENT VISIBLE HERE. Study's Practice and Projects'
//   Update and Decision now compose no titled block at all between the banner
//   and the prose — the whole of the chrome on those three pages is the banner,
//   and every row in this table lost a line.
//
...
describe("the arrangement the presets ship (5.20)", () => {
  ...
  // the state row is the first thing in the group. `row`, `stack` and `tab` are
  // the grammar of the fence rather than a block, so they are named separately
  // by the group's own test in `journal-rows.test.ts`.
  //
  // AND THE SECTION HEADS WELDED INTO IT (5.28, second half). The note's index
  // is composed into the banner's fence now, so counting fences alone would
  // report every one of these notes as one block called `journal-header` and
  // this table would stop saying which section each preset ships. A welded
  // section is still a section and its head is still the line that opens it —
  // what changed is which fence the line is in. `header:2:` heads are the
  // GROUPS inside one section (a kind per table on a deepest index) and are not
  // blocks, which is the same distinction the level rule draws when rendering.
  const blocks = (text: string): string[] => {
    ...
    for (const line of text.split("\n")) {
      ...
      if (!open) continue;
      if (line.trim() === "row" || line.trim() === STACK_KEYWORD) continue;
      if (want) {
        out.push(line);
        want = false;
        continue;
      }
      if (/^header:(?!\d+:)/.test(line)) out.push(line);
    }
    return out;
  };
