...
describe("section bodies are marked for the surface", () => {
  ...
  it("gives the logging grid a class no banner shares", () => {
    const widgets = readSrc("widgets");
    expect(widgets).toContain('out.push("ca-journal-tracker-section");');
    expect(widgets).toContain(
      "trackerSection: hasTrackerRegion && !isOverviewCard,"
    );
    // AND NOT INSIDE A STACK (5.28). The frame is the one 4.21 gave a grid that
    // had just been moved OUT of a card; a grid stacked into the banner has
    // never left one, and taking both is a card inside a card.
    expect(widgets).toContain("if (drew.trackerSection && !drew.stack) {");
  });
  ...
  it("stops the bar painting a second background inside the surface", () => {
    ...
    //
    // THE SELECTOR IS SPELLED IN FULL, ancestor included: the bare
    // `.ca-journal-sec-l1.ca-journal-header-bar {` it used to search for is a
    // SUFFIX of every scoped rule about that bar, and 5.28's stack rule — which
    // sets padding and margin and no background — is now the first of them in
    // the bundle. The test read that rule instead and reported a missing
    // `background: none` that had not moved.
    const css = readCss();
