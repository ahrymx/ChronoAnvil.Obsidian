...
// ── the shipped reference and the registry ──────────────────────────────
//
// REPOINTED IN 5.2, FROM A FILE THAT IS NEVER THERE. This block used to read
// `docs/reference.md` behind `existsSync`, and `docs/` is gitignored — so in a
// fresh clone, in CI, and on this machine the file was absent, `describe.skip`
// ran, and the three assertions below reported as skipped for every release
// they existed. They are the 3 skips the suite has been carrying. A parity
// check that cannot run is worse than none: it reads as covered.
//
// `assets/documentation.md` is the file that was wanted anyway. It ships — it
// is compiled into main.js and written into the reader's vault by the scaffold
// — so it is the reference a reader actually holds, it is committed, and a
// widget missing from it is missing from the only documentation anyone gets.
describe("the shipped reference and the registry agree on what exists", () => {
  // NOT GENERATED FROM EACH OTHER, and that is deliberate: the essay-length
  // rows in `assets/documentation.md` are the documentation's value, and a
  // generator would either destroy them or need them as input, at which point
  // it is not a generator. What is asserted is PARITY of the keyword sets, in
  // both directions — which kills a row for a widget that no longer exists and
  // forces a row for one that has been shipping undocumented, without this test
  // having any opinion about the prose in between.
  const reference = repoFile("assets/documentation.md");
  const table = reference.slice(
    reference.indexOf("## ChronoAnvil widgets"),
    reference.indexOf("## Trackers")
  );
  // The keyword is the first backticked word of a bullet, up to the first
  ...
  it("names nothing that does not dispatch and is not retired", () => {
    ...
    //
    // `frame` and `row` join it in 5.2, for the same kind of reason and stated
    // the same way: neither is a widget at all. They are lines ABOUT the block,
    // read before the directives, and the documentation says so in its own
    // first words for each. A reader types them, so they have rows.
    //
    // `stack` joins them in 5.28 as `row`'s other direction — the same kind of
    // line, dropped from the dispatch loop the same way. The list is the
    // MODIFIER keywords plus `header`, and it is still written out rather than
    // computed: `MODIFIER_KEYWORDS` also holds `cell`, `tab`, `wide` and
    // `height`, which are lines about a widget inside a block and are
    // documented inside the `row` bullet rather than as rows of their own.
    //
    ...
    // is the job `RETIRED_WIDGETS` does in the renderer and the reference does
    // here.
    const dead = [...documented]
      .filter((k) => !["header", "frame", "row", "stack"].includes(k))
