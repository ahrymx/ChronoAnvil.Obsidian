...
> {
  ...
  // ── THE SAME TITLE, FOR THE OTHER ARRANGEMENT (5.29) ────────────────
  //
  // The bar a section wears standing alone, kept for one purpose: a weld drops
  // it on the way into its host's fence, because inside that fence there is no
  // card of its own to title — the banner IS the head. `composeSectionRuns`
  // has dropped exactly this line since 5.28 and asks `soloBarOf` for it; this
  // is the same answer carried on a section that has already been bound to its
  // context, so `regroupFlatNote` can drop it when the READER welds rather than
  // only when the composer does.
  //
  // WHY IT IS NOT `bar`, WHICH HOLDS THE SAME STRING ON THREE CATALOGUES. That
  // field is the title a cell TAKES BACK when a group is broken up, and the
  // journal catalogue sets it to nothing on purpose: there, the only section a
  // cell can be is one whose bar the READER took off, so handing it back on the
  // way out would reverse their answer (see `bindSection`'s `keepBar`). A weld
  // needs the declared line on the way IN and must not hand it back on the way
  // out — the journal repair does that, and it is the one thing that knows
  // whether the reader wanted the bar at all. Two directions, two fields.
  stackBar?: string;
...
// ── WHAT MAY BE WELDED INTO A BANNER, ON ANY SURFACE (5.30) ─────────────
//
// ONE LIST, AND THE ARGUMENT FOR IT IS THE CONSTANT ABOVE IT. `BANNER_ID` is
// spelled once because five catalogues use it and a sixth spelling would be a
// section repair could not find; this is that sentence about the OTHER end of
// the same arrangement. A stack is a section folded into the banner's fence, the
// host is `banner` on every surface that has one, and there is nothing
// per-surface in the question — so there is nothing per-surface in the answer.
//
// WHAT IT REPLACES, AND WHY NOT NINE DECLARATIONS. `JournalSection.stack` was a
// field on one catalogue's type naming the host, and the shape a second
// catalogue would have taken is a second copy of it. This codebase has already
// paid for that shape twice: 4.19.1 fixed the "🔗 Links" head on the surface it
// was reported on and left it wrong on the other two, and `JOURNAL_BANNER_KINDS`
// exists because *three places ask "is this a banner?" and a literal in each is
// how the second spelling gets forgotten in one of them*. A list is also one
// thing a sweep can check against all ten catalogues at once, which
// `test/section-model.test.ts` does; nine declarations can only be swept nine
// times.
//
// IDS, NOT (SURFACE, ID) PAIRS, because an id already means one thing
// everywhere here — `trackers` is the logging grid on a journal note and on a
// diary entry, `banner` is the page's name on all ten. The day that stops being
// true this widens to pairs, which is a widening rather than a rewrite.
//
// A SECTION IN THIS LIST IS OFFERED THE WELD ONLY WHERE ITS SURFACE HAS A
// BANNER TO WELD INTO, which `viewFor` and `bindSection` check rather than this
// list stating — the same division `stacks` itself draws: the catalogue says
// *may*, the file says *is*.
export const WELDS_INTO_BANNER: ReadonlySet<string> = new Set([
  // Journal notes, since 5.28: the logging grid, and the table of what is below
  // this note under either of its two names.
  "trackers",
  "children",
  "pages",
  // Dashboards, 5.30.
  //
  // `search` is the whole reason its page exists, so the box under the page's
  // name is where a reader looks for it first.
  "search",
  // `launcher` is the most banner-like section in the tree — four tiles that
  // navigate. It ships inside `HOME_TOP_ROW`, and a section already sharing a
  // block is offered no weld (see `renderStackJoin`), so on a stock homepage it
  // has to leave that row first. That refusal is deliberate and stays.
  "launcher",
  // ── AND `contents` IS NOT HERE, WHICH IT WAS FOR HALF A RELEASE ────────
  //
  // A journal dashboard's "what's below" — *every subject in Study, and what is
  // inside it* — is `children` one surface over and was the first entry written
  // here, for `children`'s own reason. It came back out because it CANNOT be
  // welded, and the way it fails is silent: the section titles itself with
  // `frame: section` (`journal-dashboard-sections.ts`) rather than with a
  // `header:` bar, and `frame:` is a statement about a BLOCK. Welded, that line
  // lands in the banner's fence and describes the banner's block — so
  // `chromeClasses` answers `is-unframed` for the whole thing and the card the
  // stack exists to draw is not drawn at all. The page loses its box.
  //
  // THE GRAMMAR ALREADY KNEW THIS SHAPE and refuses it one arrangement over:
  // `isSectionFence` keeps a `frame: section` fence out of a row's columns
  // because *"the modifier stays behind with the block being emptied, so the
  // section loses its bar, its title and its fold in one move"*. A weld empties
  // the block the same way; the modifier travels instead of staying, which is
  // the same defect pointed the other direction.
  //
  // THE WAY IN IS A ONE-FIELD CHANGE AND IT IS NOT THIS RELEASE'S TO MAKE: give
  // the section a `header:` title instead of `frame: section` and it welds like
  // every other entry here. That changes what a journal dashboard composes and
  // therefore what it LOOKS like today, on every vault that has one — a
  // decision about the page rather than about the stack.
  //
  // `test/journal-stack.test.ts` welds every id in this list for real and reads
  // the fence back, which is what found this rather than a vault would have.
]);
...
export function viewOf<Ctx>(
  ...
  return {
    ...
    // ── AND THAT DAY CAME (5.30) ──────────────────────────────────────
    //
    // This was a POLICY for one release, with a note saying *"`stack` is
    // declared on `JournalSection` and on no other catalogue's type … the day a
    // second catalogue declares one, the field moves up beside `repeatable` and
    // this line goes."* Three more sections weld now, on three more surfaces,
    // and the field did not move up — the DECLARATION did, into
    // `WELDS_INTO_BANNER` above, where one list answers for all ten catalogues.
    //
    // NO TEST HERE FOR WHETHER THE PAGE HAS A BANNER, and none is needed. A
    // section may say it welds into the banner on a note that has no banner
    // section; the editor's own guard settles that (`renderStackJoin` offers
    // the weld only where the host is among the rows and is live), which is the
    // division `stacks` already draws — the catalogue says MAY, the file says
    // IS.
    //
...
export function insertionPoint(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    // A CHUNK CANNOT BE SPLIT, so a section whose rank falls INSIDE one lands
    // after it (5.28). The weld puts `banner` and `children` in one fence with
    // `trackers`' rank between them; ranking by the last alone read that as
    // "this chunk outranks you" and put the re-added grid ABOVE the banner,
    // which is the one place it must never go. Before this there was no chunk
    // whose ids straddled another section's rank — a row's cells are adjacent
    // in the catalogue — so the case was unreachable rather than decided.
    if (Math.min(...ranks) < rank && rank < Math.max(...ranks)) {
      after = i;
      continue;
    }
...
export function bindSection<Ctx, Opts extends object>(
  ...
  return {
    ...
    // ALWAYS, WHATEVER `keepBar` SAYS. The two fields answer opposite
    // questions — see `Section.stackBar` — and a catalogue that declines to
    // hand a title back is not declining to drop one.
    ...(soloBarOf(section, ctx) !== undefined
      ? { stackBar: soloBarOf(section, ctx) }
      : {}),
