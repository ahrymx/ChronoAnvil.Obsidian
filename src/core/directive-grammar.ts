...
// ── THE STACK — SECTIONS UNDER EACH OTHER IN ONE CARD (5.28) ─────────────
//
// `row` LAYS CELLS BESIDE EACH OTHER; A STACK PUTS SECTIONS UNDER EACH OTHER.
// They are the two arrangements a fence can hold more than one section in, and
// the reader named this one:
//
//   row   = cells beside each other
//   stack = sections under each other
//
// WHAT IT IS FOR, AND WHY THE FILE SAYS IT NOW. 5.28 welded a journal note's
// logging grid and its index into the banner's fence so the banner's chevrons
// open something ATTACHED to the banner rather than a card floating under it.
// The catalogue declared the weld and the file said nothing, which meant the
// only thing that knew three sections were one card was the code that composed
// them — a reader reading their own note saw a fence whose shape was a secret,
// and a fence somebody typed by hand could not ask for the arrangement at all.
// The line is the answer to both.
//
// NO ARGUMENT, FOR `row`'s REASON. The members ARE the directives under it: a
// count would be a number that has to agree with them, and the host is the
// first section in the fence because a stack is what it says — under each
// other, in order. An argument is refused rather than ignored.
//
// ── AND ONE LINE PER SECTION, WHICH REVERSES 5.28 (5.29) ─────────────────
//
// The line went in ONCE, at the top, and the refusal below said why: *a block
// is one stack — the sections in it are already under each other, so a second
// line describes nothing.* That is true of the ARRANGEMENT and false of the
// FILE, and the difference cost a reader the way back out.
//
// A row divides its cells with `cell`. A stack divided its sections with
// nothing at all, so which lines belonged to which section was knowable only by
// the code that had just composed them — and `flatBlocks` answers that question
// with `hasKnownExtent`, which says no for anything rendering more than one
// line. On a journal note that is the logging grid and the index, which is to
// say every member of every stack this plugin composes. So **Break up the
// stack** was drawn disabled, over the arrangement 5.29's own weld button had
// just made, with a tooltip about lines that cannot be told apart. The reader
// found it the day it shipped.
//
// The honest fix is the one a row already has: SAY where each section starts.
// Each `stack` line opens a section in the stack — the first one opens the host
// and declares the fence, every later one opens the section under the last. The
// two spellings a reader might reach for are the same file, exactly as they are
// for `cell`: a line above the first directive and a line between each pair
// come out identical, because the top line is the first section's opener.
//
// WHAT IT IS NOT. It is not a layout instruction — the renderer has dropped
// every `stack` line from its dispatch since 5.28 and still does, so a divided
// stack and an undivided one draw the same card. It exists because the file has
// to be able to say what only the composer knew, which is the argument that put
// the first line there.
//
// AN UNDIVIDED STACK IS STILL A STACK, and this is what makes the change a
// minor one rather than a break: a fence carrying the 5.28 spelling, or a word
// somebody typed at the top of a fence of their own, reads as one part holding
// every section in it. Nothing renders differently; the sections in it simply
// cannot be told apart, which is exactly what was true of every stack before
// this. `stackParts` is where that degrades, and it degrades to the answer the
// old code gave.
//
// A STACK IS NOT A ROW AND A FENCE MAY NOT BE BOTH. Two arrangements over one
// set of directives is two answers to where the second one goes, and the
// renderer would have to break the tie silently. The refusal names the way out,
// which is the way out of every other one-per-fence rule here: a second fence.
export const STACK_KEYWORD = "stack";

// What a fence's `stack` line said, or why it could not be honoured.
//
// `stack: false` ON AN ERROR, which is `parseRow`'s fallback and the same
// argument: a refused line costs a reader their arrangement and never their
// content.
...
// Whether this line is the stack modifier rather than a directive.
//
// EXACT, for `isRowLine`'s reason one keyword over: a future `stack-something`
// directive must not be swallowed by a prefix test.
export function isStackLine(line: string): boolean {
  return splitDirective(line).keyword === STACK_KEYWORD;
}

export function parseStack(lines: readonly string[]): StackSpec {
  const stackLines = lines.filter((l) => isStackLine(l));
  if (stackLines.length === 0) return { stack: false, error: null };

  // EVERY ONE OF THEM IS CHECKED, not just the first. A stack's lines are one
  // per section now, so `stack: 2` written on the third of them is as much a
  // refusal as one written on the first — and reading only `stackLines[0]`
  // would have honoured a fence carrying a value nobody could see.
  const bad = stackLines.find((l) => splitDirective(l).argument.trim());
  if (bad) {
    const raw = splitDirective(bad).argument.trim();
    return {
      stack: false,
      error: `stack takes no value, so \`stack: ${raw}\` is refused. A stack holds every section in its block, one under the next, in the order they are written.`,
    };
  }

  // BOTH LINES IS NOT A LAYOUT, IT IS A QUESTION NOBODY ANSWERED. See the head
  // of this section: the renderer would have to pick one in silence.
  if (lines.some((l) => isRowLine(l))) {
    return {
      stack: false,
      error:
        "This block is both a row and a stack. A row lays its cells beside each other and a stack puts its sections under each other — pick one, and put the other arrangement in its own ```chronoanvil block.",
    };
  }

  return { stack: true, error: null };
}

// Where each section in a stacked fence begins and ends, in body line numbers.
//
// ONE PART PER `stack` LINE, running from that line to the next one — the
// divider travels with the section it opens, which is what makes a part
// something that can be lifted out whole and leave nothing of itself behind.
//
// THE LINES BEFORE THE FIRST `stack` LINE ARE NOBODY'S, and there is normally
// nothing there: the composer writes the word above the host's first line and
// the weld target writes it at the top of the fence it opens. A reader who put
// a `frame:` line above it has written something about the whole block, and it
// stays with the block rather than with its first section.
//
// AND A FENCE WITH ONE `stack` LINE HAS ONE PART, holding every section in it.
// That is the 5.28 spelling and the answer for it is the honest one: the
// sections are together, where they begin is not written down, and a caller
// asking which lines are whose is told there is one region rather than being
// given a guess. `flatBlocks` reads exactly that and calls them not loose.
export function stackParts(
  body: readonly string[]
): { from: number; to: number }[] {
  const opens: number[] = [];
  body.forEach((line, at) => {
    if (isStackLine(line)) opens.push(at);
  });
  return opens.map((from, i) => ({
    from,
    to: i + 1 < opens.length ? opens[i + 1] : body.length,
  }));
}
