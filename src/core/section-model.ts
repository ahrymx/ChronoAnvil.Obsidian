...
export interface SectionView {
  ...
  // ── WHAT THIS SECTION MAY BE WELDED UNDER (5.29) ────────────────────
  //
  // The id of the section whose fence this one's lines may be folded into, or
  // absent — which is every section on three surfaces and most of them on the
  // fourth. 5.28 gave the journal catalogue a `stack` field naming the host;
  // this is that field said to the editor, so the window can offer the
  // arrangement the composer has been making since.
  //
  // ON THE VIEW RATHER THAN A LIST THE WINDOW KEEPS, which is `category`'s
  // argument in this file a release ago and `group`'s and `movable`'s before
  // that. The alternative was the editor holding the two ids it is allowed to
  // weld — and an editor that knows `trackers` and `children` by name has
  // learned which surface it is on, which is the one thing this interface
  // exists to prevent. What the reader is offered is now something the
  // CATALOGUE says.
  //
  // IT SAYS *MAY*, NOT *IS*. Whether these sections are actually in one fence
  // is a fact about the file and arrives through `BlockView.stack`; this is a
  // fact about the section and is true of a section sitting on its own at the
  // bottom of the note.
  stacks?: string;
...
export interface SectionModel {
  ...
  // can.
  // `stacks` NAMES THE SECTIONS THAT ARE WELDED under the one above them
  // rather than sitting beside it — 5.29, and the fourth argument is the fourth
  // thing a block can be asked. `blocks` says which sections share a fence and
  // cannot say which of 5.28's two arrangements that fence is; a row lays its
  // cells beside each other and a stack puts its sections under each other, and
...
export interface BlockView {
  ...
  pages: string[];
  // Whether this block is a STACK rather than a row — 5.28.
  //
  // WHAT THE WINDOW DOES WITH IT: it names the card. Every shared block was a
  // row when this interface was written, so the card says "Group" and its
  // button says "Break up the group", and a journal note's banner-grid-index
  // block is neither of those things — its members are under each other, which
  // is what the reader asked to have a word for.
  //
  // READ FROM THE FENCE, like `pages` and `column` and for their reason: it is
