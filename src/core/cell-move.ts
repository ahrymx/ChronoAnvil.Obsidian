...
export interface CellSource {
  ...
  from: number;
  to: number;
  // HOW MANY LINES AT THE FRONT OF THE RANGE ARE THE BLOCK'S AND NOT THE
  // WIDGET'S (5.29). They are REMOVED with the run and do not travel with it.
  //
  // One caller, one line: a section leaving a stack takes its part, and a part
  // begins with the `stack` line that opens it. That line says where the
  // section starts INSIDE the fence, and a section that has left is not inside
  // it any more — carried along it would arrive at the top of a fence of one,
  // declaring an arrangement made of a single section.
  //
  // NOT `from + 1` WITH THE LINE LEFT BEHIND, which is the other way to write
  // this and the wrong one: the divider has to go from the source too, or the
  // fence keeps an opener with nothing under it and the part above it swallows
  // whatever comes next.
  drop?: number;
}
...
export type CellTarget =
  ...
  // UNDER EVERYTHING BLOCK `block` HOLDS, IN ITS FENCE, WITH NO DELIMITER —
  // which makes that block a stack. 5.29, and it is `group`'s twin: the same
  // one kind that changes what the target block IS rather than where something
  // sits inside it, for the other of 5.28's two arrangements.
  //
  //   row   = cells beside each other, divided by `cell`
  //   stack = sections under each other, divided by nothing
  //
  // NO LINE AND NO SIDE, because a stack has neither to name. A column arrives
  // at a boundary and the caller has to say which one; a welded section arrives
  // at the END, always, because the block's opener is the section whose fence
  // this is — see `weldInto` in row-order.ts, which is the same sentence said
  // about the editor's arrangement.
  //
  // AND IT IS THE ONE KIND THE PAGE HEAD'S BLOCK ACCEPTS. See the 4.11 rule
  // below for why that exception exists and what it is still refusing.
  | { kind: "weld"; block: number }
...
const STRUCTURE = new Set<string>([
  ...
  WIDE_KEYWORD,
  HEIGHT_KEYWORD,
  // 5.29, and it is the `tab` line's argument one keyword later. `stack` landed
  // in 5.28 and was not added here, so every count in this file read a stacked
  // banner as holding one more widget than it does — and, worse, a `stack` line
  // on its own answered `isContent` and could therefore be lifted out as a
  // "run" into a fence of its own. Nothing a reader could press reached it
  // (the arrangement was composed, never moved), which is exactly the kind of
  // latent arithmetic 5.29 turns into a gesture.
  STACK_KEYWORD,
]);
...
export function moveCell(
  ...
  if ((src.drop ?? 0) < 0 || src.from + (src.drop ?? 0) >= src.to) return null;
  const run = srcBody.slice(src.from + (src.drop ?? 0), src.to);
  ...
  // ── AND `weld` IS THE ONE EXCEPTION, WHICH 5.28 EARNED (5.29) ────────
  //
  // A STACK IS A SECTION WELDED INTO THE FENCE THAT HOLDS THE PAGE'S NAME, and
  // that is what it is FOR: a journal note's logging grid and its index are
  // attached to the banner so the banner's chevrons open something joined to
  // it rather than a card floating under it. `composeSectionRuns` has written
  // exactly that fence since 5.28 — this rule never saw it, because composing
  // a note builds the fence rather than moving lines into one.
  //
  // So the arrangement existed and only the COMPOSER could make it: a reader
  // whose note was written before the weld, or who broke one up, had no gesture
  // that could put it back. 5.29 is that gesture, and this is the line it needs.
  //
  // WHAT IS STILL REFUSED, AND IT IS THE WHOLE OF 4.11's RULE BUT ONE CLAUSE.
  // The head is not a SOURCE (the `run` test above is unchanged, so its
  // directive cannot be lifted out or carried anywhere). Nothing may be
  // inserted ABOVE it. And the three merging kinds that could DISPLACE it —
  // `group`, which would make its fence a row; `cell`, which opens a column
  // beside it; `swap`, which would trade it for something else — are refused
  // exactly as they were. `weld` can do none of those three: it appends at the
  // end of the body, so the head keeps its fence, its position and its place at
  // the top of the block it opens.
  //
  // AND `swap` IS NOT WIDENED WITH IT, WHICH WAS TRIED AND PUT BACK. A stack's
  // members sit inside the head's block, so reordering two of them looks like a
  // swap of two runs neither of which is the head — and phase three of
  // `regroupFlatNote` performs that swap ONE LINE AT A TIME, which is right for
  // a column holding a single directive and wrong for a section that spans a
  // header, a region and a table. Allowed through, it would have traded a
  // section's anchor line and left the rest of it where it was. Refused, a
  // reorder inside a stack writes nothing and is reported as nothing, which is
  // what the documentation already says happens and what a reader can see.
  ...
  } else if (dst.block === headBlock && dst.kind !== "weld") {
    return null;
  }
  ...
  // TWO BLOCKS BECOME ONE STACK. 5.29.
  //
  // `group`'s TWIN, LINE FOR LINE, and every difference between them is a
  // difference between the two arrangements:
  //
  //   THE KEYWORD IS `stack` AND IT GOES IN ONCE. `composeSectionRuns` writes
  //   it at the top of the fence the moment the first member lands and finds it
  //   already there for the second — so this looks before it writes, and two
  //   ways of making one object write one file. (`group` never has to: a row is
  //   made out of two blocks and can therefore only be made once.)
  //
  //   THE DELIMITER IS THE SAME WORD (5.29). A row divides its cells with
  //   `cell`; a stack divides its sections with another `stack` line, and that
  //   line is what says where the arriving section starts. So the run is
  //   appended under one — and there is still no `arrival` rule beyond that,
  //   because a stack has exactly one place to arrive at, which is the end.
  //
  //   AND THE DESTINATION MAY ALREADY BE ONE. A row is refused a second column
  //   opened this way because the side would name a boundary nobody pointed at;
  //   a stack has no boundaries, so a third section welds on under the second
  //   exactly as the second welded under the first.
  //
  // WHAT IT REFUSES. A fence that is a row (`parseStack` refuses a block that
  // claims both, so the write must not produce one), a fence of another kind
  // (checked above, for every destination), and its own source block.
  if (dst.kind === "weld") {
    if (dst.block === src.block) return null;
    const dstBody = bodyOf(segs[at[dst.block]]);
    if (dstBody.some(isRowLine)) return null;
    ...
    // ABOVE THE FIRST LINE THAT IS ALREADY THERE, which is where `rowRuns`
    // writes `row` and where `insertStackLine` puts this one on the repair
    // path — a modifier describes the whole fence, and a reader looking for
    // what a block is looks at its first line.
    const opened = dstBody.some(isStackLine)
      ? [...dstBody]
      : [STACK_KEYWORD, ...dstBody];
    edited.set(at[dst.block], [...opened, STACK_KEYWORD, ...run]);
    return rebuild(lines, segs, edited, -1, srcFence, run);
  }
  ...
  // now cannot.
  //
  // AND PAST THE LAST BLOCK IS NOT THE END OF THE FILE (5.29). `dst.at` counts
  // BLOCKS and a note is not made of blocks — between and after them sit the
  // reader's prose, the skeleton, the graph trailer. `segs.length` reads
  // "become the block after the last one" as "go to the bottom of the note",
  // and every segment in between moves above the arrival without anybody asking
  // for it.
  //
  // WHERE IT WAS FOUND. Breaking a stack up asks for exactly this: each section
  // leaves the banner's fence to become the block after it, and on a note with
  // one fence `dst.at` is 1 with `at.length` 1 every time. The reader's grid and
  // index came out UNDER their writing and under the graph comment. The APART
  // fixture that proves the round trip has nothing after its fences, so end-of
  // -file and after-the-last-fence were the same index there and the assertion
  // passed on a note that could not show the difference.
  //
  // A NEW BLOCK GOES WHERE ITS NEIGHBOUR IS, in both directions: before the
  // fence it displaces, or directly after the last one. `rebuild` puts the
  // blank lines round it.
  const newAt =
    dst.kind === "block"
      ? dst.at < at.length
        ? at[dst.at]
...
function rebuild(
  ...
  for (let i = 0; i <= segs.length; i++) {
    if (i === newAt) {
      // ── AND IT IS SEPARATED FROM BOTH ITS NEIGHBOURS (5.29) ────────
      //
      // The trailing blank was unconditional and there was no leading one,
      // which held while a new block only ever landed where a blank already
      // was — before another fence, or at the end of a note whose fences were
      // the last thing in it. A block landing directly under the fence it left
      // has neither: the two fences touch, and the blank goes below the pair
      // instead of between them. Asked of the lines rather than of the
      // segments, because what matters is what the note READS as.
      if (out.length && out[out.length - 1].trim() !== "") out.push("");
      out.push(...wrap(srcFence[0], run));
      if (segs[i]?.[0]?.trim() !== "") out.push("");
    }
