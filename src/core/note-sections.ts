...
export function flatBlocks(
  ...
  return parseFlatSections(text, sections)
    ...
    .map((run) => {
      ...
      // AND WHICH ARRANGEMENT THIS FENCE SAYS IT IS (5.28). Not derived from
      // the shape — a stack and a row are both "more than one section in one
      // fence", and the only thing that tells them apart is the line the fence
      // carries.
      const isStack = body.some(isStackLine);
      // ── AND WHETHER IT SAYS WHICH LINES ARE WHOSE (5.29) ──────────────
      //
      // `hasKnownExtent` is a question about the CATALOGUE — does this section
      // render one line — and it is the right question for a row, where the
      // only thing dividing two cells may be the fact that each is one
      // directive. A stack answers it "no" for every member worth welding: the
      // logging grid is a region and the index is a head and a table.
      //
      // A DIVIDED STACK ANSWERS IT FROM THE FILE INSTEAD, and that is not a
      // guess of the kind `sharersIn` refuses at length. The `stack` line above
      // a section's lines is a fact somebody wrote down — the composer, the
      // weld target, or a reader — and it says where that section begins as
      // plainly as a `cell` line says where a column does.
      //
      // WHAT MAKES IT TRUSTWORTHY IS THE BIJECTION, and it is checked rather
      // than assumed. Every section in the fence must have an anchor, every
      // anchor must fall inside a part, and no two may fall inside the same
      // one. A fence carrying the 5.28 spelling has one part holding three
      // anchors and fails on the third clause — so it degrades to exactly the
      // answer it got before this existed, which is what keeps an undivided
      // stack safe rather than merely unsupported.
      const parts = isStack ? stackParts(body) : [];
      const partOf = (id: string): number => {
        const line = run.lineOf[id];
        // BODY-RELATIVE, which `whereIs` says out loud one function down: a run
        // counts its lines from the opening fence and `parts` are indices into
        // the body, so the marker comes off here. Reading the two numberings as
        // one puts every anchor a line late and the last one past the end.
        if (line == null) return -1;
        const body0 = line - 1;
        return parts.findIndex((p) => body0 >= p.from && body0 < p.to);
      };
      const seats = run.sectionIds.map(partOf);
      const divided =
        parts.length >= run.sectionIds.length &&
        seats.every((seat) => seat !== -1) &&
      ...
      return {
        ...
        loose:
          run.sectionIds.length === 1 || divided
            ? [...run.sectionIds]
...
function whereIs(
  ...
  // AND THE ANCHOR WITHOUT THE EXTENT QUESTION (5.29). `cellLineIn` answers
  // "which line may be CUT", so it withholds the number from a section whose
  // extent is a guess — which is right for every caller that goes on to cut one
  // line. A section in a divided stack is cut by its `stack` line rather than
  // by its extent, and the anchor is how phase one finds which part is its; the
  // two questions are separated here rather than by weakening the first.
  const raw = run.lineOf[id];
  return {
    block,
    line: at0 === null ? null : at0 - 1,
    anchor: raw == null ? null : raw - 1,
    body,
  };
}
...
export function regroupFlatNote(
  ...
  // WHICH MEMBERS ARE WELDED RATHER THAN BESIDE (5.29), in the terms `pages`
  // already uses: a list of sections, each naming itself rather than its block.
  // `undefined` means LEAVE THE ARRANGEMENT ALONE, which is `pages`' convention
  // and matters for the same reason — a caller written before stacks existed
  // must not have its rows quietly welded, and one that passes an empty list is
  // saying something different ("this note has none").
  const welded = new Set(stacks ?? []);
  ...
  // AND WHICH OF THEM WERE WELDED RATHER THAN BESIDE (5.29), read in the same
  // walk and for the same phase. A section leaving a STACK gets back the title
  // the weld took off it, which is `stackBar` and not `bar` — the two fields
  // answer opposite questions and the journal catalogue answers them
  // differently on purpose. Read from the file the note arrived as, because
  // that is the arrangement the section is leaving; the arrangement it is
  // going to has no bar to give it.
  const wasWelded = new Set<string>();
  for (const b of flatBlocks(text, sections)) {
    for (const id of b.ids) wasWith.set(id, b.ids.length);
    if (b.stack) for (const id of b.ids.slice(1)) wasWelded.add(id);
  }
  ...
  for (let pass = 0; pass < ceiling; pass++) {
    ...
    const id = leaving[leaving.length - 1];
    const at = whereIs(lines, sections, id);
    if (!at || at.anchor == null) break;
    // ONE LINE, OR THE WHOLE PART IT SITS IN (5.29). A cell of a row is one
    // directive and its anchor IS its extent — that is what `hasKnownExtent`
    // let through and the only thing phase one could move until now. A section
    // welded into a stack is a `stack` line and everything under it as far as
    // the next one, which is a range the FILE states; `flatBlocks` has just
    // called it loose on the strength of that line, so this reads the same
    // line rather than a second opinion about it.
    //
    // AND THE OPENER GOES WITH IT, which is `drop`. The divider says where the
    // section starts inside the fence, and a section leaving the fence is not
    // inside it — see `CellSource.drop` for why it may not simply stay behind.
    //
    // ASKED OF A FENCE THAT IS ACTUALLY DIVIDED, and the count is the guard.
    // An UNDIVIDED stack is one part holding everything, so a member of it that
    // is loose the old way — a section that happens to render one line — would
    // otherwise have its "part" read as the whole body and carry the banner out
    // with it. Fewer parts than sections means the file has not said where the
    // boundaries are, and the one-line answer is the only honest one left.
    const block = now.find((b) => b.ids.includes(id));
    const divided = stackParts(at.body).length >= (block?.ids.length ?? 2);
    const part = divided
      ? stackParts(at.body).find(
          (p) => at.anchor! >= p.from && at.anchor! < p.to
        )
      : undefined;
    if (!part && at.line == null) break;
    const src = part
      ? { block: at.block, from: part.from, to: part.to, drop: 1 }
      : { block: at.block, from: at.line!, to: at.line! + 1 };
    const next = moveCell(lines, src, { kind: "block", at: at.block + 1 });
  ...
  // stays.
  //
  // ── AND A WELD DROPS THE ONE THE CATALOGUE DECLARED (5.29) ───────────
  //
  // Same splice, one field over. `bar` is the title a cell takes BACK and the
  // journal catalogue withholds it deliberately; `stackBar` is the title a
  // welded section gives UP, which is a different question with a different
  // answer on exactly that catalogue. Reading `bar` alone left the reader's
  // weld carrying a line the composer's weld drops — two ways of making one
  // object writing two files, which `cell-move.ts` says out loud is the thing
  // not to allow.
  for (const b of want) {
    for (const id of b.slice(1)) {
      const section = sections.find((s) => s.id === id);
      const bar = welded.has(id) ? section?.stackBar ?? flatBar(section) : flatBar(section);
      if (!bar) continue;
      const at = whereIs(lines, sections, id);
      if (!at) continue;
  ...
  for (let pass = 0; pass < ceiling; pass++) {
    ...
    //
    // ── AND A WELD LIFTS THE WHOLE FENCE, NOT A WIDGET RUN (5.29) ─────
    //
    // `widgetRun` is the COLUMN rule and it refuses two things a column may not
    // be: a fence that titles itself, and a fence holding more than one widget.
    // Both refusals are right for a row — a column's head would render below
    // the group it titles — and both are wrong for a stack, where a member
    // keeps whatever head it composes and the note's index is a head, a button
    // and a table in one fence. Asked here, the weld would have been refused on
    // every section a reader would ever want to weld.
    //
    // WHAT BOUNDS IT INSTEAD IS THE BLOCK. A section alone in its fence IS its
    // fence, so the run is the whole body and no extent has to be guessed —
    // which is the same fact `flatBlocks` states from the other end when it
    // calls such a section loose whatever its lines are. A section sharing a
    // fence is left alone: it is already in a block with something, and moving
    // it is phase one's business.
    const mine = now.find((b) => b.ids.includes(joining));
    if (welded.has(joining) && mine?.ids.length !== 1) break;
    const run = welded.has(joining)
      ? { from: 0, to: from.body.length }
      : widgetRun(from.body);
    if (!run) break;
    ...
    // ── WHICH ARRANGEMENT THE READER ASKED FOR (5.29) ─────────────────
    //
    // The two branches below build a ROW: an arrival into one that already has
    // columns, or the pair of blocks that becomes one. A weld is the other
    // shape and it is chosen here rather than derived, because nothing in
    // either file can tell them apart — two sections in one fence is a row or a
    // stack depending only on what the reader wanted, which is what `stacks`
    // carries.
    //
    // FIRST, so the row arithmetic is never consulted about a fence that is not
    // going to be one. `weld` refuses a destination that already carries `row`
    // for the same reason `parseStack` refuses to read one.
    const dst: CellTarget = welded.has(joining)
      ? { kind: "weld", block: to.block }
  ...
  // ── PHASE SIX: WHAT THE FENCE SAYS IT IS (5.29) ─────────────────────
  //
  // The `weld` target writes `stack` the moment the first member lands, exactly
  // as the `group` target writes `row`. This is the other direction and the
  // reason the write cannot be left to the target alone: a member that LEAVES a
  // stack is a `moveCell` out of the fence, and nothing in that move knows the
  // line at the top of the fence it left describes an arrangement that is no
  // longer there. `pruned` cannot answer it either — it drops `row`, `cell`,
  // `tab` and `height` from a fence that has come down to one WIDGET, and a
  // banner alone in its fence is two widgets (its name and its navigation) and
  // no stack.
  //
  // SO IT IS ASKED OF THE SECTIONS, WHICH IS THE ONLY PLACE THE ANSWER IS. A
  // stack is several SECTIONS in one fence; the count of directives is not the
  // same question and never was.
  //
  // AND IT WRITES THE LINE ON A FENCE THAT WAS ALREADY TOGETHER. A note
  // composed between the weld shipping and the keyword landing carries the
  // arrangement and not the word for it (see `missingStackLine`, which reports
  // exactly that as a repair) — so a Save that touches nothing else still
  // leaves the file saying what it is, rather than leaving the window to call
  // the reader's stack a group next time it opens.
  //
  // `undefined` IS LEFT ALONE, on `pages`' rule and for its reason.
  // ── AND IT IS ASKED OF THE FENCE, NOT OF THE WISH ──────────────────
  //
  // THE WORD GOES IN where the reader asked for a stack and the fence is
  // shared. It comes out only where the fence has stopped being shared at
  // all — never merely because the arrangement was un-asked-for.
  //
  // THAT ASYMMETRY IS THE POINT, and getting it wrong costs a reader their
  // repair. Not every section can be cut out of a fence it shares: `loose` is
  // false for anything whose extent is a guess, which on a journal note is the
  // logging grid and the index — so *break up the stack* is refused for them,
  // the sections stay in one fence, and the block goes on being one card. A
  // rule that stripped the line on the strength of the ask would take the word
  // off a fence that is still, in every way a reader can see, a stack.
  //
  // AND IT WOULD FIGHT `applySections`. The two passes run in one Save, in that
  // order, and the repair that writes a missing `stack` line into a note
  // composed before the keyword is in the first of them. Taking the line back
  // out here would undo, every time, the one thing that Save had to do.
  if (stacks) {
    for (const block of flatBlocks(lines.join("\n"), sections)) {
      const at = whereIs(lines, sections, block.ids[0]);
      if (!at) continue;
      const has = at.body.some(isStackLine);
      const shared = block.ids.length > 1;
      const wants = shared && block.ids.slice(1).some((id) => welded.has(id));
      const body = shared
        ? wants && !has
          ? [STACK_KEYWORD, ...at.body]
          : null
        : has
          ? at.body.filter((l) => !isStackLine(l))
          : null;
      if (!body) continue;
      const next = replaceBlockBody(lines, at.block, body);
      if (next) lines = next;
    }
  }

  // ── PHASE SEVEN: THE TITLE A CELL TAKES BACK (5.14) ─────────────────
  //
  ...
  // invalidate. Phase six sits between them for the same reason: it rewrites
  // one modifier on a fence whose contents are settled, and moves nothing.
  for (const block of flatBlocks(lines.join("\n"), sections)) {
    if (block.ids.length !== 1) continue;
    const id = block.ids[0];
    if ((wasWith.get(id) ?? 1) < 2) continue;
    const was = sections.find((s) => s.id === id);
    // THE TITLE THE WELD TOOK OFF, GIVEN BACK BY THE SAME NAME (5.29). The drop
    // between phases one and two reads `stackBar` for a section being welded;
    // this reads the same field for one that has just stopped being welded, so
    // the two are one rule read in two directions. Reading `flatBar` here alone
    // left a journal note's grid coming out of a stack with no title at all —
    // `keepBar: false` withholds `bar` on that catalogue, deliberately, for a
    // question this is not.
    const bar = wasWelded.has(id) ? was?.stackBar ?? flatBar(was) : flatBar(was);
    if (!bar) continue;
    const at = whereIs(lines, sections, id);
    if (!at) continue;
...
export function flatNoteModel(spec: FlatNoteSpec): SectionModel {
  ...
  return {
    ...
    regroup: (text, blocks, pages, stacks) =>
      regroupFlatNote(text, sectionsFor(text), blocks, pages, stacks),
  };
}
