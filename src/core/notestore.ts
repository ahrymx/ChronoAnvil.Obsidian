...
// separate marker comments) means Obsidian never renders it in Reading mode —
// comments are dropped natively — so the reading view needs no plugin-side
// hiding. The widget is the sole reader/writer; the raw file still carries the
// content in plain text, just parked in a comment, so it survives without the
// plugin.
//
// LIVE PREVIEW WAS THE HALF THIS PARAGRAPH GOT WRONG, for six releases and
// until 5.31. It named the editor alongside the reading view, and the editor is
// not one: Obsidian drops a comment it PARSED, and in Live Preview these three
// lines are text like any other, so every note with a region drew its markers
// and its contents as source. `ui/marker-lines.ts` is the CM6
// decoration this paragraph said was unnecessary — it hides the whole comment,
// which is the extent reading mode takes — and source mode still shows it.
//
