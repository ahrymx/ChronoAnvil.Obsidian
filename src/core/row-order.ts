...
// the four functions that say so in their names — `joinInto`, `weldInto`,
// `takeOut`, `breakUp` — and every other operation restores the boundaries it
// found. The first two also MOVE a row, because a block is a run and a row
// cannot join one it is not next to; they differ in what the block BECOMES,
// which is the third bit and is 5.29's whole subject.
//
// ── AND IT IS PURE ───────────────────────────────────────────────────────
//
...
export interface Arrangement {
  ...
  // ── THE THIRD BIT, AND IT SAYS HOW THE BLOCK IS ARRANGED (5.29) ──────
  //
  // This row is WELDED UNDER the one above it rather than sitting beside it.
  // `joined` says these rows share a fence and cannot say which of the two
  // shapes that fence is — 5.28 named them: a `row` lays its cells beside each
  // other, a `stack` puts its sections under each other, and the file tells
  // them apart by one line.
  //
  // A SUBSET OF `joined`, ON `paged`'s TERMS: a row that is not in a block with
  // anything cannot be welded to it.
  //
  // AND ALL-OR-NOTHING PER BLOCK, which `paged` is not and which is the only
  // invariant this bit adds. `parseStack` refuses a fence that is also a row,
  // so half a stack is not a shape the file can hold — a block whose second
  // member is welded is a stack, and `normalise` marks the rest of it rather
  // than letting the window describe an arrangement no write could make.
...
export function joinInto(
  ...
  paged.delete(id);
  return normalise(restack(arr.rows, band, order), joined, paged, arr.stacked);
}

// ── THIS ROW IS WELDED UNDER THE BLOCK `anchor` OPENS (5.29) ───────────
//
// `joinInto`'s TWIN, AND THE DIFFERENCE IS ONE WORD IN THE FENCE. That one adds
// a column beside what is already there; this one adds a section UNDER it, and
// the file says which by carrying `row` or `stack`. Everything else — the row
// travels to the block, the bits are set rather than inferred, a block of one
// is the only thing that may go — is the same and is stated there.
//
// IT ALWAYS ARRIVES AT THE END, which is the one place `joinInto`'s rule cannot
// be borrowed. A join comes in through the NEAREST edge so that take-out and
// re-join are a round trip; a weld has no such symmetry, because the block's
// opener is the section whose FENCE this is — the banner, on a journal note —
// and a member landing in front of it would be claiming to own a fence it is
// being folded into. So a row above its host travels down past it, and a stack
// is read top to bottom in the order the reader can see.
//
// THE HOST IS A BLOCK, NOT A SECTION, and `anchor` names any row of it: a
// second widget welding onto a banner that already holds the logging grid is
// joining the block those two are in, which is what "stack onto the banner"
// means once there is a stack. The chain `composeSectionRuns` follows on the
// compose side is the same fact read from the file.
export function weldInto(
  arr: Arrangement,
  band: readonly string[],
  id: string,
  anchor: string
  ...
  if (tgt < 0 || tgt === at) return null;
  const rest = blocks.filter((_, i) => i !== at);
  const to = tgt > at ? tgt - 1 : tgt;
  const order = rest.flatMap((b, i) => (i === to ? [...b, id] : [...b]));
  const joined = new Set(arr.joined);
  joined.add(id);
  const paged = new Set(arr.paged);
  // A WELDED SECTION IS NOT A PAGE. Pages divide a row's columns and a stack
  // has none — `parseTabs` refuses a `tab` line in a fence with no `row` — so
  // the bit comes off here rather than being written and refused on the way to
  // disk.
  paged.delete(id);
  const welds = new Set(arr.stacked);
  welds.add(id);
  // AND THE SECOND MEMBER IS WHAT `normalise` READS. Welding onto a block that
  // is already a group would leave that block half one shape and half the
  // other; marking its second member is how this says "the whole fence is a
  // stack now", and it is the same bit the reader would have set by welding
  // each of them in turn.
  const block = rest[to] ?? [];
  if (block.length > 1) welds.add(block[1]);
  for (const x of block.slice(1)) paged.delete(x);
  return normalise(restack(arr.rows, band, order), joined, paged, welds);
}
...
export function takeOut(
  ...
  const paged = new Set(arr.paged);
  paged.delete(id);
  // AND IT COMES OUT UNWELDED. A row that leaves a stack is a card of its own
  // again, so the bit goes with it — `normalise` would keep it otherwise, on
  // the block it is joining or opening, and a lone row claiming to be welded
  // under nothing is exactly the half-fence this bit's invariant refuses.
  const welds = new Set(arr.stacked);
  welds.delete(id);
  return normalise(restack(arr.rows, band, order), joined, paged, welds);
}
...
export function breakUp(
  ...
  for (const x of block) {
    ...
    paged.delete(x);
    // A GROUP THAT IS NOT A GROUP IS NOT A STACK EITHER, and for `paged`'s
    // reason one bit along: the card is gone, so the arrangement it was in is
    // gone, and a bit left behind would weld the members straight back
    // together the moment anything put two of them next to each other again.
    welds.delete(x);
  }
  return normalise(arr.rows, joined, paged, welds);
}
...
// them.
// AND THE WELD IS SETTLED BLOCK BY BLOCK (5.29). The page bit is filtered —
// a boundary on a row that is in no group divides nothing — and the weld bit
// cannot be, because it describes the FENCE rather than the row: a block whose
// second member is welded is a stack, and every other member of it is welded
// too whether or not anybody said so. So this SPREADS rather than filters, and
// spreading is what makes "a fence is a row or it is a stack" structural
// instead of something each of the operations below has to remember.
//
// FROM THE SECOND MEMBER, which is the one member of a block that cannot be
// ambiguous: the opener is never marked (it is what the others are welded
// under, exactly as it is what they are joined to), and a bit on the third
// member of a block whose second one is a column would be describing half a
// fence.
export function normalise(
  ...
  for (const block of blocksOf(rows, joined)) {
    ...
    if (block.length > 1 && stacked.has(block[1])) {
      for (const id of block.slice(1)) welds.add(id);
    }
  }
  const pages = new Set<string>();
  for (const id of paged) if (out.has(id)) pages.add(id);
  return { rows: [...rows], joined: out, paged: pages, stacked: welds };
}
