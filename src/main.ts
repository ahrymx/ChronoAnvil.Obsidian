...
export default class ChronoAnvilPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    // AND IN EDIT MODE ITS FENCES STAY DRAWN. 5.31, and the plugin's first
    // editor extension. Live Preview hands a block back to the editor whenever
    // the cursor is inside it, which on a diary entry is the first thing under
    // the cursor after a Ctrl+E; the guard makes each fence atomic so the
    // cursor goes around it instead. Source mode is untouched — see
    // ui/fence-cursor.ts, where that is the whole design.
    this.registerEditorExtension(fenceCursorGuard());

    // AND ITS MARKERS STAY OUT OF SIGHT. The other half of the same sentence,
    // and the half the report was about: a region comment, the skeleton's two
    // bracket lines, the graph block and the spacer's code span all render as
    // nothing in reading view and were drawn as literal text in Live Preview.
    // Registered separately from the guard above because it is a different
