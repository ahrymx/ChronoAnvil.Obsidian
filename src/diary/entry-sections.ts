...
const ENTRY_DECLS: EntryDecl[] = [
  ...
  },
  {
    ...
    // there.
    //
    // AND THE JOURNAL CATALOGUE REVERSED THAT IN 5.28, DELIBERATELY NOT HERE.
    // The same split was made in both catalogues in the same release for the
    // same reason; screenshots of a Study Lesson measured what it costs on a
    // note whose grid is two fields — a whole card boundary for a head, a
    // border and a gap — and the journal side welded the grid back into the
    // banner's fence, keeping it a separate SECTION the reader can untick. The
    // decision is at `journal-sections.ts`'s `trackers` entry, with the
    // measurement.
    //
    // AN ENTRY IS NEXT AND IS NOT THE SAME EDIT. Its banner has a footer case
    // the journal banner has no equivalent of (`:has(> .ca-journal-entry-context)`),
    // the strip named in the blurb above sits between the two, and the reader
    // asked to see the journal version in the vault first. Deferred on purpose:
    // when it happens, the argument to reverse is the one above, in this file.
    //
