...
// here" is now answered — which since 1.0.23 is all three of them, every journal
// having a page surface.
...
export async function openTemplateEditor(
  ...
  // A `modelWith` AND A `structuralHere` STOOD HERE UNTIL 1.0.23. They built a
  // second model — the catalogue as it would be once the `pages` row was ticked
  // — so that the window could preview, plan and save a section whose own gate
  // still said it did not apply. The gate says yes on every leaf now, so the
  // ordinary model offers the row and there is no second one to build. See
  // `JournalPages`.
