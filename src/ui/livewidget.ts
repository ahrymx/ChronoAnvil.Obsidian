...
// Fired on the host after every build, the initial one included.
//
// WHY AN EVENT AND NOT A CALLBACK. What a rebuild can invalidate is not this
// widget's business — the first case is an empty index whose sentence the
// SECTION HEAD is carrying (`ui/empty.ts`), and the head belongs to the
// dispatcher, several frames up a switch that has no way to reach in here with
// a closure. An event on the host is the one thing both sides already hold, it
// dies with the element, and a listener nobody registers costs nothing.
export const LIVE_REDRAW_EVENT = "ca-live-redraw";
