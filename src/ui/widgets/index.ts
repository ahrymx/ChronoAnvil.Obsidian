...
import { liftEmptyHead } from "../empty";
import {
  belowSpanIn,
  revealPartsIn,
  RevealBar,
...
import type { RevealStore } from "../reveal";
...
export interface BlockComposites {
  ...
  // ── A STACK: SEVERAL SECTIONS IN ONE CARD, UNDER EACH OTHER (5.28) ──
  //
  // The `stack` line's rendered half. A journal note's banner, its logging grid
  // and its index are one fence now, and the card they share is the STACK's —
  // not the banner's figure and not the tracker section's frame, both of which
  // were written for a block holding one thing.
  //
  // AND IT IS WHY `trackerSection` GOES QUIET. That class is the frame 4.21
  // gave a grid that had just been moved OUT of a card; a grid inside a stack
  // has never left one, and taking it as well is 4.1 §3.1's two borders
  // arguing — a card inside a card, which is exactly what the vault render of
...
export function chromeClasses(
  ...
  // A STACK'S CARD REPLACES THE TRACKER SECTION'S, and the two are exclusive
  // here rather than at the call site — `chromeClasses` is the rule and the
  // caller is its application, which is what the frame comment above already
  // says about every other class in this list.
  if (drew.trackerSection && !drew.stack) {
    out.push("ca-journal-tracker-section");
  }
  if (drew.stack) out.push("ca-journal-stack");
  return out;
}
    ...
    EntryControlHost
{
  ...
  register(): void {
    ...
    this.registerBlock("chronoanvil", (source, el, ctx) => {
      ...
      // AND WHETHER IT IS ONE STACK (5.28) — `row`'s other half, read in the
      // same place and dropped from `lines` for the same reason. A row lays its
      // cells beside each other; a stack puts its sections under each other in
      // one card, which is what a journal note's banner, its logging grid and
      // its index now are. `parseStack` refuses a fence that claims both.
      const stackSpec = parseStack(
        rawLines.filter((l) => l.length > 0 && !l.startsWith("#"))
      );
      ...
      //
      // AND IT IS `isPageHeadLine`, WHICH IS 5.11's FOURTH CALLER, FOUND IN
      // 5.28 (from a vault: *"drag icon should never appear on a banner
      // stack"*). That release listed the three keywords a page carries its own
      // name under — `title`, `journal-header`, `entry-header` — and fixed the
      // three refusals in `cell-move.ts` and `flatBlocks` that had all been
      // keyed on `title:` alone. This was the fourth, and it was the one a
      // reader could SEE: a journal note's banner and a diary entry's are page
      // heads, so both blocks were pinned in the section editor and draggable on
      // the page.
      //
      // WHAT MADE IT VISIBLE WAS THE STACK. The grip goes on the block's first
      // `header:` bar, or on the block itself where there is none — so the
      // banner's grip used to sit on the banner's own top edge, where a reader
      // reads it as the banner's and mostly leaves it alone. Fold the index into
      // the same fence and the first bar is "🗂️ Topics": the dots landed
      // mid-card over a section's title, offering to drag the note's head by a
      // handle sitting on something else.
      const fixed = drawable.filter(({ l }) => isPageHeadLine(l)).map(({ at }) => at);
      ...
      if (stackSpec.error) {
        container.createDiv({
          cls: "ca-journal-frame-error",
          text: `ChronoAnvil: ${stackSpec.error}`,
        });
      }
      if (cellSpec.error) {
        container.createDiv({
      ...
      for (const [n, line] of lines.entries()) {
        ...
        // a title and a sentence naming the button above it. Composed as a
        // SECTION that button is in the bar four pixels away wearing the very
        // label the sentence quotes, so the box is the head said twice at three
        // times the height — 185px of card, on the state a fresh lesson is in.
        //
        // ASKED HERE RATHER THAN DECIDED IN THE WIDGET, because whether there
        // is a head to lift onto is this loop's knowledge and nothing the
        // builder can see: the same `pages-table` answered `widget` by
        // `formQuestion` composes with no `header:` line at all, and there the
        // callout is the only thing on the card and stays exactly as it was.
        // `liftEmptyHead` returns false for that case and touches nothing.
        liftEmptyHead(widget, headerGroup);

        // AND AGAIN EVERY TIME THE WIDGET REDRAWS ITSELF. All three of these
        // tables are live-scoped, so adding the first page rebuilds the table
        // under a bar still saying "none yet" — the phrase and the rows would
        // be two answers to one question, and the one on the bar is the one a
        // reader reads first. `liftEmptyHead` is a sync rather than a one-shot
        // for exactly this call; the listener is on the widget's own element
        // and dies with it.
        if (headerGroup) {
          const bar = headerGroup;
          widget.addEventListener(LIVE_REDRAW_EVENT, () =>
            liftEmptyHead(widget, bar)
          );
        }
      ...
      //
      // WHY IT IS HERE AND NOT IN THE LOOP. What a reveal needs to know is what
      // the WHOLE FENCE turned out to be — a banner, the note's index, the
      // tracker section — and a loop over directives learns that one line at a
      // time. This is the same place the frame is chosen for the same reason,
      // and like the frame it reaches back into nothing that was drawn: it
      // registers two facts and appends one strip.
      //
      // NOT ON A MANAGED TEMPLATE. A template's preview is the arrangement
      // itself, so a template that opened with everything hidden would be a
      // window showing none of what it is for — the same exemption the tracker
      // add-tile takes three hundred lines up.
      //
      // ── AND THE BANNER REVEALS WHAT SHARES ITS FENCE, AND NOTHING ELSE
      //    (5.29) ─────────────────────────────────────────────────────
      //
      // THE REPORT, the day the break-up shipped: *"two new images after I
      // broke up a stack group. The trackers and whats below sections seem to
      // be stuck in the stack format, they should return to being normal
      // sections/widgets."* They were. 5.28 registered a target from a block of
      // its own as well — the shape it described as *"a reader who dragged the
      // section out of the banner"* — and a reveal is CLOSED BY DEFAULT and
      // draws nothing at all when it is closed. So the two cards a break-up had
      // just made came back not drawn, with two chevrons in the banner as the
      // only way to see them: the stack's chrome, on a note that no longer had
      // a stack.
      //
      // 5.28 had no gesture that could take a section out of the banner's
      // fence, so that branch was written for a shape only a hand-edit could
      // reach and never met the one it was named after. 5.29's break-up is that
      // gesture, and it has to MEAN something — a card of its own is a card of
      // its own, wearing its own head and its own fold, which is the control
      // every other section on every other surface has.
      //
      // So the rule is one sentence: a reveal belongs to a section WELDED INTO
      // the banner. Nothing is needed to release what was hidden — a target is
      // visible until a controller claims it, and a block that no longer
      // registers is a block nothing claims.
      //
      // ── AND ANY PAGE WITH A BANNER, NOT ONLY A JOURNAL NOTE (5.30) ───
      //
      // THE READER'S ASK: *"I think the stack could become a global thing for
      // any page with a banner."* The weld itself was already surface-blind —
      // the grammar, the move, the editor's three gestures — and this strip was
      // the last journal-shaped piece of it: two hard-coded words, each with a
      // hand-written rule for finding its card.
      //
      // `stackParts` REPLACES BOTH RULES, which is what 5.29's dividers were
      // for. A divided fence states where each of its sections begins, so the
      // parts after the host ARE the reveals — one chevron each, in the order
      // they are written, labelled by the head the section wrote. Nothing here
      // knows what a tracker grid is or what a "what's below" is any more.
      //
      // TWO FENCE TESTS, AND THE NARROWER ONE IS THE OLD PATH. `BANNER_KINDS`
      // is every page head this plugin draws — a dashboard's `title`, a diary
      // entry's `entry-header`, a journal note's `journal-header` — and that is
      // the reach a DIVIDED stack gets, because a divided stack was written by
      // this release's own gestures and says what it is. The UNDIVIDED reading
      // below it stays on `JOURNAL_BANNER_KINDS`, where it has always been: it
      // is 5.28 compatibility — a fence welded before the dividers existed —
      // and 5.28 composed stacks on journal notes and nowhere else. Widening it
      // would hand a diary entry, whose trackers have sat in the banner's fence
      // since long before any of this, a chevron nobody asked for.
      const fenceKinds = lines.map((l) => l.split("|")[0].split(":")[0].trim());
      ...
      if (!isManagedTemplate(this.plugin, ctx.sourcePath)) {
        ...
        // is what the card head calls it everywhere else, and every other
        // widget is what `frame: section` would title a fence of it.
        const parts = isBannerFence
          ? revealPartsIn(rawLines, (kind) =>
              kind === "tracker" ? TRACKER_BAR_TITLE : SECTION_TITLES[kind]
            )
          : [];
        ...
        // ── AND THE 5.28 SPELLING, WHICH HAS NO DIVIDERS TO READ ────────
        //
        // A journal note welded before 5.29 carries ONE `stack` line, so
        // `stackParts` gives one part holding every section and `revealPartsIn`
        // answers nothing — the same degradation `flatBlocks` makes on the same
        // fence, and for the same reason: where the sections begin is not
        // written down. The two hand-written rules are kept for exactly that
        // fence and reached only when the general one found nothing.
        //
        // THE GRID, AND ONLY WHERE THE BANNER HOLDS IT. `chromeClasses`
        // states the rule this follows: *a fence is a banner or it is the
        // tracker section*. Inside the banner the grid is a band of somebody
        // else's card, so the band is what a chevron takes; a fence of its own
        // IS the tracker section, paints its own card and carries its own
        // title, and the banner has no business hiding it.
        ...
        if (trackerBar && undivided) {
          ...
          ctx.addChild(
            new RevealTargetChild(trackerBar, ctx.sourcePath, {
              id: "trackers",
              label: TRACKER_BAR_TITLE,
              el: trackerBar,
            })
          );
        }

        // WHAT'S BELOW, WHERE IT IS WELDED INTO THE BANNER — which is what the
        // catalogue composes since the reader looked at the first cut and said
        // *"trackers is ok but whats below is not attached to the banner
        // section"*. There is no block of its own to hide then: the section is a
        // head and the widgets under it, siblings inside the banner's card, so
        ...
        // AND THE STRIP THAT DRIVES THEM, on the banner and nowhere else. A
        // note has one banner; a second strip would be a second set of controls
        // over one set of sections, disagreeing the moment either is pressed.
        //
        // UNDER THE NAME AND ABOVE THE GRID. The banner reads name → controls →
        // content, which is the order 3.7 settled for the entry card's own
        // footer in the opposite direction: chrome that ACTS on what follows
        // goes before it, chrome that acts on the card goes under it.
        //
        // AND ONLY WHERE THERE IS SOMETHING TO REVEAL (5.29), which is the
        // reader's *"the banner should lose the chevron buttons"* said in the
        // DOM rather than in the stylesheet. Every target a strip can ever show
        // is registered above, from this fence, in this pass — so a banner that
        // registered none will never have one, and the empty `<div>` it used to
        // append was a control the note had no use for, kept off the page by a
        ...
        if (isBannerFence && revealAnchors.length) {
          ...
          // grid — drew its chevrons UNDERNEATH the section they open, because
