...
import { emptyCallout, emptyHead, emptyLine } from "./empty";
...
export function folderRollup(
  ...
  if (topicFolders.length === 0) {
    ...
    // AND THE HEAD MAY TAKE IT INSTEAD (5.28). The callout above is built
    // unchanged and is what a reader sees wherever this table is the whole of
    // its own card; on a subject index it is composed under `header:` and
    // `button:<type>:new-container`, and the dispatcher lifts this phrase onto
    // that bar and drops the box. See `emptyHead`.
...
export function kindTable(
  ...
  if (notes.length === 0) {
    ...
    // `header:`, `button:` and this table per kind, and on a topic nobody has
    // written into yet that is a stack of boxes each holding one sentence and a
    // button that is already in the bar four pixels above it. See `emptyHead`.
    return emptyHead(root, "none yet");
  }
...
export function buildPagesTable(
  ...
  if (pages.length === 0) {
    ...
    // AND THE THIRD TABLE THE HEAD MAY SPEAK FOR (5.28). This is the card the
    // screenshots caught at its worst: a fresh lesson's Pages section is
    // `header:`, `button:` and this table, and with no pages yet the whole card
    // is one sentence naming the button four pixels above it. The two rollups
    // got the lift in the same release and this one was missed, which is the
    // shape of miss `emptyHead`'s own comment describes — it names "a subject's
    // Topics table, a lesson's Pages table" and only the first was wired.
    //
    // The line above still stands, because the lift is the DISPATCHER's to
    // make: a `pages-table` answered "Show as widget" composes with no `header:`
    // at all, and there the sentence is the only thing on the card.
    return emptyHead(root, "none yet");
  }
