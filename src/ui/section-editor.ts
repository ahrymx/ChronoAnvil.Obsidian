...
export class SectionEditorModal extends EditorModal {
  ...
  // Which rows are WELDED under the one above them rather than sitting beside
  // it — 5.28 for the word, 5.29 for the bit.
  //
  // IT BEGAN AS NAMING AND NOTHING ELSE. 5.28 read the arrangement out of the
  // file so the card could say *Stack* rather than *Group*, because a card
  // calling a journal note's banner-and-index a group is the window using the
  // vocabulary of the arrangement it is NOT. Every control on it worked the
  // same way on either, so one set keyed by opener was enough.
  //
  // IT IS THE THIRD BIT NOW, BECAUSE THE READER CAN CHANGE IT. *"the section
  // editor needs a way for users to initiate the banner as a stack"* — so the
  // window no longer just reports the arrangement, it writes one, and a bit
  // that is written has to survive every move the other two survive.
  // `row-order.ts` owns it beside `joined` and `paged`, keyed by MEMBER as
  // those two are: a set keyed by opener goes stale the moment a block's first
  // row changes, and `normalise` has no way to notice.
  //
  ...
  constructor(app: App, plugin: ChronoAnvilPlugin, private spec: SectionEditorSpec) {
    ...
    // READ ONCE, because the weld check below asks about every member of every
    // block and `sections()` builds a catalogue each time it is called.
    const stacksUnder = new Map(
      spec.model.sections(spec.text).map((s) => [s.id, s.stacks])
    );
    for (const block of spec.model.blocks?.(spec.text) ?? []) {
      for (const id of block.ids.slice(1)) this.joined.add(id);
      // AND WHETHER THEY ARE WELDED OR BESIDE EACH OTHER (5.28). One fact per
      // block in the file, one bit per member here — `normalise` keeps the two
      // spellings equal, and the member is the row a control is drawn on.
      //
      // OR THE CATALOGUE SAYS SO (5.29). A note composed between the weld
      // shipping and the keyword landing carries the arrangement and not the
      // word for it, so the fence answers `stack: false` about three sections
      // that are demonstrably one card. Left at that the window would call the
      // reader's stack a group, offer to break it up in those words, and hand
      // the Save a `welds` list that disagreed with the repair running beside
      // it. Asking the sections is `isStackRun`'s own rule — *would we compose
      // these as a stack* — and it answers for both spellings.
      const said =
        block.stack ||
        (block.ids.length > 1 &&
          block.ids.slice(1).every((id) => stacksUnder.get(id) !== undefined));
      if (said) for (const id of block.ids.slice(1)) this.stacked.add(id);
  ...
  // Whether this block is a stack rather than a group — asked of its second
  // member, which is where `normalise` writes the answer. 5.29.
  //
  // THE SECOND MEMBER RATHER THAN ANY OF THEM, and it is the same rule the
  // arrangement normalises to: a fence is a row or it is a stack, so the one
  // member that cannot be ambiguous decides for all of them. A block of one is
  // neither, and answers false without a special case.
  private isStack(group: readonly string[]): boolean {
    const kept = group.filter((id) => !this.removed.has(id));
    return kept.length > 1 && this.stacked.has(kept[1]);
  }
  ...
  private welds(ids: readonly string[]): string[] {
    return this.groupsOf(ids).flatMap((group) =>
      group.filter((id, i) => i > 0 && this.stacked.has(id))
    );
  }
  ...
  private layoutOps(): SectionOp[] {
    ...
    for (const [id, opener] of to) {
      ...
      ops.push({
        ...
        sectionId: id,
        label,
        // AND IT NAMES THE ARRANGEMENT IT IS JOINING (5.29). "Joins one block
        // with the banner" is true of a weld and describes a row, which is the
        // one thing the sentence has to get right: the reader is reading this
        // list to check the press they just made did what its button said.
        detail:
          opener === id
            ? `${label} takes a block of its own`
            : this.stacked.has(id)
              ? `${label} is welded into the ${this.view(opener)?.label ?? opener} card`
  ...
  private renderBlock(host: HTMLElement, group: readonly string[]): void {
    // THE WORD FOR WHAT THIS CARD IS (5.28). A group's members sit beside each
    // other; a stack's sit under each other, in one card, which is the
    // arrangement a journal note's banner, logging grid and index are in. One
    // noun, used everywhere this card names itself, so the card and its buttons
    // cannot end up describing two different objects.
    const noun = this.isStack(group) ? "stack" : "group";
    const card = host.createDiv({ cls: "ca-tpl-block" });
    // AND THE CARD SAYS IT WITHOUT WORDS AS WELL. The wash and the direction it
    // runs in are the stylesheet's; what belongs here is only which of the two
    // this is. See `.ca-tpl-block.is-stack` for the argument.
    card.addClass(noun === "stack" ? "is-stack" : "is-group");
    ...
    bar.createSpan({
      ...
      //
      // AND A STACK HAS NO PAGES TO COUNT — `parseStack` refuses a fence that
      // is also a row, and pages are a row's division — so the count clause is
      // a group's alone rather than being written for both and never reached.
      text:
        noun === "stack"
          ? "Stack"
          : pages.length > 1
    ...
    if (split.disabled) {
      // ── AND IT SAYS WHICH REFUSAL THIS IS (5.29) ──────────────────────
      //
      // THE REPORT: *"One of these widgets' lines can't be told apart from
      // others in its block, so the group can't be broken up", is the tooltip I
      // get when trying to break up a stack group.* Two things were wrong with
      // it. The sentence called the reader's stack a group, in the one place
      // the card beside it calls it a stack; and the way out it named — move it
      // out of the block by hand — is the operation this button IS.
      //
      // A stack has a real answer now, and it is the one the file can act on:
      // the fence divides its sections with a `stack` line above each of them,
      // so a stack that cannot be broken up is one composed before 5.29, and
      // saving the note offers to write the missing lines.
      split.title =
        noun === "stack"
          ? "This stack doesn't say where each section starts, so it can't be broken up. Save the note — the plan offers to add the missing `stack` lines — and try again."
  ...
  // The stack controls for one row: the host's, or an approved widget's. 5.29.
  //
  // ONE METHOD FOR BOTH, because they are two halves of one question — *is this
  // row's stack there yet* — and splitting them would let the two halves
  // disagree about which rows count. Every rule about who may be welded is
  // asked once, here, of the arrangement the Save will write.
  private renderStackJoin(
    lead: HTMLElement,
    section: SectionView,
    band: readonly string[]
  ): void {
    // THE ROWS THAT WOULD WELD UNDER THIS ONE, and the row this one would weld
    // under. Both read from `SectionView.stacks`, which is the catalogue's
    // field and the only thing either branch knows about ids.
    //
    // ASKED OF THE ROWS THAT WILL BE WRITTEN. A section on its way out is not
    // in the file the Save produces, so a stack built out of it would be a card
    // the reader was promised and would not get — `joinables` states the same
    // rule for groups and this is it one arrangement over.
    const live = (id: string): boolean =>
      !this.removed.has(id) && this.unanswered(id).length === 0;
    const guests = this.rows.filter(
      (id) => live(id) && this.view(id)?.stacks === section.id
    );
    const host = section.stacks;

    if (host && this.rows.includes(host) && live(host)) {
      // A GUEST. It is offered the join only once the host's block IS a stack:
      // making one is the host's button, above, and offering both would be two
      // controls for one press with nothing to choose between them.
      const block = blockOf(band, this.joined, host);
      if (!this.isStack(block) || block.includes(section.id)) return;
      if (blockOf(band, this.joined, section.id).length !== 1) return;
      const name = this.view(host)?.label ?? host;
      const b = lead.createEl("button", {
        cls: "ca-tpl-arrow ca-tpl-weld",
        attr: {
          "aria-label": `Stack onto ${name}`,
          title: `Weld this section into the ${name} card, under what is already in it — it loses its own card and its own title bar, and moves with the ${name}`,
        },
      });
      setIcon(b, "layers");
      b.addEventListener("click", () => {
        this.settle(weldInto(this.arrangement, band, section.id, host));
      });
      return;
    }

    // A HOST, and only while it has guests to take and is not already a stack.
    if (!guests.length) return;
    const mine = blockOf(band, this.joined, section.id);
    if (this.isStack(mine)) return;
    // AND ONLY GUESTS THAT ARE FREE TO COME. One already in a block with
    // something else is in an arrangement the reader made, and pulling it out
    // of that to build this would be a press that changes two things.
    const taking = guests.filter(
      (id) => blockOf(band, this.joined, id).length === 1
    );
    if (!taking.length) return;
    const names = taking.map((id) => this.view(id)?.label ?? id);
    const list =
      names.length > 1
        ? `${names.slice(0, -1).join(", ")} and ${names[names.length - 1]}`
        : names[0];
    const b = lead.createEl("button", {
      cls: "ca-tpl-arrow ca-tpl-weld ca-tpl-weld-host",
      attr: {
        "aria-label": `Make this a stack`,
        title: `Make this a stack — weld ${list} into this card, under what is already in it. Each one loses its own card; they open and close with this one.`,
      },
    });
    setIcon(b, "layers");
    b.addEventListener("click", () => {
      // IN ORDER, AND EACH ONE ONTO THE HOST. `weldInto` moves the row to the
      // end of the host's block, so welding them in list order is what puts
      // them in the card in the order the reader is looking at — and every
      // press after the first finds a block that is already a stack, which is
      // the same arrangement the second button makes one at a time.
      //
      // AND THE BAND IS RE-READ FROM THE WORKING ARRANGEMENT EACH TIME, not
      // from `this`. A weld MOVES a row, so the band's ORDER after the first
      // press is not the order on screen — and `weldInto` works in blocks cut
      // from the band it is given. Its membership does not change (nothing
      // leaves the band, and every guest was in it), so the set is taken once
      // and the order comes from the arrangement being built.
      const inBand = new Set(band);
      let next: NextArrangement | null = null;
      let arr: Arrangement = this.arrangement;
      for (const id of taking) {
        const here = arr.rows.filter((x) => inBand.has(x));
        const step = weldInto(arr, here, id, section.id);
        if (!step) continue;
        next = step;
        arr = step;
      }
  ...
  private renderRow(host: HTMLElement, section: SectionView): void {
    ...
    if (this.hasRows && !gone) {
      ...
      // surprise came from.
      // ── THE STACK, WHICH THE WINDOW COULD ONLY REPORT (5.29) ────────
      //
      // THE REQUEST: *"the section editor needs a way for users to initiate the
      // banner as a stack … then approved widgets (for now that is trackers and
      // what's below) can be linked into the stack."*
      //
      // 5.28 built the arrangement and only the COMPOSER could make one. A
      // reader whose note was written before the weld, or who took the logging
      // grid out of the banner and wanted it back, had nothing to press — the
      // card said *Stack*, the button on it broke one up, and there was no
      // button anywhere that made one. That is the same defect 4.53.0 fixed for
      // groups a release at a time, arriving one arrangement later.
      //
      // TWO GESTURES, ONE PER STATE, WHICH IS THE READER'S OWN SENTENCE. The
      // host's button INITIATES — it is drawn only while there is no stack, and
      // it takes in every approved widget at once, because "make my banner a
      // stack" is one decision rather than three. Each approved widget's button
      // JOINS — drawn only once a stack exists, for a widget that is not in it.
      // So exactly one of the two is ever available for a given pair, and
      // neither is a second door onto the other's operation.
      //
      // WHICH WIDGETS ARE APPROVED IS THE CATALOGUE'S ANSWER, through
      ...
      if (unit === "cell") {
        ...
        // ── AND IT SAYS WHICH ARRANGEMENT IT IS LEAVING (5.29) ────────
        //
        // The card behind this row has said *Stack* since 5.28 and the button
        // on it said "the group", which is the window using both vocabularies
        // for one object — the thing 5.28 changed the card's own word to stop.
        // One noun, read off the same bit the card reads.
        const block = blockOf(band, this.joined, section.id);
        const welded = this.isStack(block);
        const noun = welded ? "stack" : "group";
        const opens = block[0] === section.id;
        const out = lead.createEl("button", {
          cls: "ca-tpl-arrow ca-tpl-leave",
          attr: {
            "aria-label": `Take out of the ${noun}`,
            title: `Take out of the ${noun} — give this section a block of its own`,
          },
        });
        setIcon(out, "unlink");
        ...
        //
        // AND A STACK'S OPENER CANNOT LEAVE ITS OWN FENCE (5.29). The others
        // are welded INTO it — on a journal note it is the banner, and the
        // banner holds the note's name — so "take this one out" is a move
        // `moveCell` refuses by name (*the page head is not a source*) and the
        // write would leave the reader's list and their file disagreeing. Break
        // the stack up instead, which is the operation they mean and is on the
        // card.
        out.disabled = !this.loose.has(section.id) || (welded && opens);
        if (welded && opens) {
          out.title = `This is the section the others are welded into — its fence is the card. Use “Break up the stack” to give them each their own again.`;
        } else if (out.disabled) {
          // AND A STACK SAYS WHERE TO GO, WHICH A ROW CANNOT (5.29). The
          // sentence is the same refusal either way — this section's lines
          // cannot be told from its neighbours' — but a stack has an answer: the
          // fence divides its sections with a `stack` line above each of them,
          // and one composed before 5.29 carries only the declaration. Saving
          // this note writes the rest where they can be worked out, which is
          // what `undividedStack` reports as an extension in the Changes tab.
          out.title = welded
            ? "This stack doesn't say where each section starts, so this one can't be split out. Save the note — the plan offers to add the missing `stack` lines — and try again."
        ...
        // the same fact, drawn.
        // AND NOT INSIDE A STACK AT ALL (5.29). A page divides a row's
        // COLUMNS — `parseTabs` refuses a `tab` line in a fence with no `row`,
        // and `parseStack` refuses a fence that is both — so the control would
        // be offering a boundary that has nothing to divide and a line the
        // reader's own note would then report as an error.
