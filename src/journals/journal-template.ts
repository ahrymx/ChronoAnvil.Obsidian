...
import { headerLevel } from "../core/directive-grammar";
...
function headerTitleIn(text: string, claims: readonly string[]): string | null {
  ...
  for (const block of fenceBlocks(text)) {
    ...
    // THE LAST TOP-LEVEL BAR ABOVE THIS SECTION'S OWN LINES, not the first bar
    // in the fence (5.28). A welded fence carries two sections' bars — the
    // banner's grid has none and the index below it has one — so "the fence's
    // first `header:`" handed the index's title to the tracker grid, and
    // saving a retitled Lesson as a layout would have renamed Trackers.
    //
    // A `header:2:` IS NOT A CANDIDATE: it is a group inside a section, not a
    // section's bar, which is the demotion rule `parseHeaderDirective` states.
    // Reading it would have given the index the title of whichever kind group
    // happened to come last.
    let titled = 0;
    let line: string | null = null;
    for (const l of block) {
      if (l.startsWith("header:")) {
        const rest = l.slice("header:".length);
        if (headerLevel(rest, titled === 0) === 1) line = l;
        titled++;
        continue;
      }
      const keyword = l.slice(0, l.indexOf(":") === -1 ? l.length : l.indexOf(":")).trim();
      if (anchors.includes(keyword)) break;
    }
    if (!line) return null;
