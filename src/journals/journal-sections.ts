...
import { BANNER_ID, WELDS_INTO_BANNER, bindSection } from "../core/sections";
...
export interface JournalSection
  ...
  // HOLDING ALL OF THEM, and the reader named the arrangement:
  //
  //   row   = cells beside each other
  //   stack = sections under each other
  //
  // The fence says so out loud — `STACK_KEYWORD`, written as its first line, on
  // exactly the terms `rowRuns` writes `row`. It did not at first: the
  // catalogue declared the weld and the file said nothing, so the only thing
  // that knew three sections were one card was the code that composed them.
  // See the keyword in `directive-grammar.ts` for the rest of that argument.
  //
  // NAMES NO HOST, AND IS NOT DECLARED HERE ANY MORE (5.30). This was
  // `stack?: string` — the id of the section whose fence this one's lines fold
  // into — on this catalogue's type and no other's. It is `WELDS_INTO_BANNER`
  // in `core/sections.ts` now: one list for all ten catalogues, because the
  // host is `BANNER_ID` on every surface that has one and there was never
  // anything per-surface in the question. See that list for why nine
  // declarations was the shape to avoid.
  //
  // WHAT STAYS TRUE OF IT, and is checked at compose time rather than declared:
  // the host must be the section whose fence the one above ended with. A reader
  // who moves Trackers down the page has said they want it somewhere else, and
  // a stack that followed it there would be the arrangement overruling them.
  // Unstacked it composes its own fence and its own bar; nothing about the
  // section is conditional on the stack except where its lines are written.
  //
  // NOT A ROW, AND THE TWO WERE NEVER THE SAME FIELD. `rowRuns` decides what a
  // `row` line means for all four catalogues and refuses to weld across one
  // (see its `weld` parameter). This is the other shape: no `cell` delimiter is
  // available, the members are under each other rather than beside, and the
  // fence is read back by `ownersBySignature` dealing signatures off the front
  // rather than by `parseRow`. `parseStack` refuses a fence that says both.
}
...
export function renderSection(
  ...
  return renderBlocks(sectionBlocks(section, ctx, opts));
}

// The same join, given blocks somebody else is holding.
//
// SPLIT OUT FOR THE WELD (5.28). `composeSectionRuns` merges the tracker grid
// into the banner's fence before anything is rendered, so the banner it emits
// is no longer what `section.render` would return — and a renderer that went
// back to the catalogue for its blocks would silently undo the merge.
function renderBlocks(parts: readonly SectionBlock[]): string {
...
export function widgetFormBar(
  ...
  // ── A WELDED SECTION HAS NO BAR OF ITS OWN TO DROP (5.28) ────────────
  //
  // The toggle's whole meaning is *draw this section's card with a head, or
  // bare*. A section written into the fence above it has no card: the host's
  // head is the only one in the block, and `withAnswers`' widget branch filters
  // every `header:` line in the chunk it is handed — so answering `section`
  // would splice `header:📊 Trackers` into the BANNER's fence, above
  // `journal-header`, and answering `widget` would take the banner's own title
  // off if it had one.
  //
  // THE WRITE PATH ALREADY REFUSED IT and this is the read path catching up.
  // `applySections` honours one form answer per fence, the OPENER's, so a
  // question arriving for the welded member was dropped on save — a control
  // that draws, flips, saves and changes nothing, which is exactly the latent
  // defect 5.21 found in the task table's scope button.
  //
  // ASKED OF THE CATALOGUE RATHER THAN OF THE PAGE, AND THAT COSTS SOMETHING.
  // A reader who drags Trackers out of the banner gets a card with a bar and no
  // toggle over it, where before this release it had one. `questionsOf` is
  // asked with a context and no text — it cannot see where on the page the
  // section landed — and threading the note through every caller to buy back
  // one toggle on an arrangement nothing ships is the wrong end of the trade.
  // The bar is still there and still renameable in place, which is the gesture
  // the toggle was a shortcut for.
  if (WELDS_INTO_BANNER.has(section.id)) return undefined;
...
export function composeSectionRuns(
  ...
  // ── THE WELD, BEFORE ANYTHING IS CHUNKED (5.28) ────────────────────────
  //
  // A section declaring `weld` writes its lines into the fence the section
  // above it ended with, and emits no block of its own. See the field for what
  // a weld is and why it is not a row.
  //
  // BEFORE THE CHUNKING, and that is the reason this is a pass rather than a
  // clause inside the loop. The banner renders a spacer AND a fence, so it is
  // not a column and never enters a chunk; the grid is a lone fence and always
  // does. Welding them where the chunker can see it would mean teaching the
  // chunk rule about multi-block sections — the fact `rowRuns` was extracted to
  // stop this file knowing. Here the two are still just lists of blocks.
  //
  // AND THE WELDED SECTION DROPS ITS BAR. `soloBarOf` is what the section would
  // have titled its own card with, and inside the host's fence there is no card
  // of its own to title: the banner IS the head. Filtered rather than made
  // conditional in `render`, because `render` cannot see whether the section
  // above it is the one it names — and a section that composed no bar and then
  // failed to weld would be a grid with no title at all.
  //
  // ── AND IT IS A CHAIN, NOT A PAIR (5.28, second half) ──────────────────
  //
  // The reader looked at the shipped arrangement and asked for the note's index
  // to be attached to the banner the way the grid is. The index does not follow
  // the banner — the GRID does, and the index follows the grid — so a rule that
  // reads "the section above me is the one I name" would refuse it on every
  // note that logs anything, which is every note.
  //
  // What the pass actually cares about is whose FENCE the section above ended
  // with, and a welded section's lines are in its host's. So the question is
  // asked of the fence rather than of the neighbour: `owner` follows the weld
  // back to the block that really holds the lines, and it is that block's
  // section the `weld` must name. The rule a reader can see is unchanged — move
  // the index below your prose and the section above it is prose, which owns no
  // banner fence, so it composes its own card exactly as before.
  const owner = new Map<number, number>();
  for (let i = 1; i < rendered.length; i++) {
    const r = rendered[i];
    if (!WELDS_INTO_BANNER.has(r.section.id)) continue;
    const at = owner.get(i - 1) ?? i - 1;
    if (rendered[at].section.id !== BANNER_ID) continue;
    const host = rendered[at].blocks;
    const into = host[host.length - 1];
    const mine = r.blocks[0];
    // ONE FENCE INTO ONE FENCE, OF THE SAME KIND. The same two conditions
    // `rowRuns` applies to a run, for the same reasons: a section with a region
    // or a markdown block has lines that cannot go inside a fence, and a
    // `chronoanvil-charts` fence could never share a block with a `chronoanvil`
    // one.
    if (r.blocks.length !== 1 || mine.kind !== "fence") continue;
    if (into?.kind !== "fence" || into.info !== mine.info) continue;
    const bar = soloBarOf(r.section, ctx);
    // AND THE FENCE SAYS WHAT IT IS. `STACK_KEYWORD` goes in at the top the
    // moment the first member lands — `rowRuns` writes `row` the same way and
    // in the same place, because a reader looking for what a block is looks at
    // its first line. A second member finds that line already there; a fence
    // nothing stacks into never gets one, which is what keeps a page's lone
    // banner exactly as it was.
    //
    // AND ONE MORE GOES IN WITH EACH MEMBER (5.29). The word divides as well as
    // declares — `cell` to this fence's `row` — so the line above a member's
    // lines is where that member starts, and the file finally says which of its
    // lines are whose. Written HERE, in the composer, because this is the pass
    // that knows: it is holding the member's own block and is about to fold it
    // into somebody else's fence, which is the only moment the boundary is a
    // fact rather than something to be worked out afterwards from anchors.
    //
    // THE TOP LINE IS THE HOST'S OPENER, not a fourth kind of line. Three
    // sections in a fence carry three `stack` lines whichever way you read them
    // — one declaration plus two dividers, or one opener each — and the two
    // readings write the same file, exactly as `cell`'s two spellings do.
    const opened = into.lines[0] === STACK_KEYWORD ? into.lines : [STACK_KEYWORD, ...into.lines];
    host[host.length - 1] = {
      ...into,
      lines: [...opened, STACK_KEYWORD, ...mine.lines.filter((l) => l !== bar)],
    };
    r.blocks = [];
    r.column = false;
    owner.set(i, at);
  ...
  for (const r of rendered) {
    // A welded section's lines are already in the block above it.
    ...
    flush();
    // `renderBlocks` RATHER THAN `renderSection`, because a host section's
    // blocks are no longer what its `render` returns — see the weld above.
    out.push(renderBlocks(r.blocks));
  }
  flush();
  return out;
}
...
//
// WHAT IT WAS. 5.18 paired the tracker grid with the stats band on a leaf index
// as ONE group with two tabs — two blocks of numbers about the same note, the
// grid being what it is graded on and the band what everything under it adds up
// to, stacked as two bordered boxes reading as one subject. It was `row: state`
// on both, `tab: true` on the band, and a conditional bar on each: the row
// carried the title, so neither cell wrote one.
//
// WHY IT CANNOT SURVIVE THE WELD. The grid is inside the banner's fence now,
// and a section cannot be in two blocks. There is no second cell left for the
// band to pair with, so `stateRow` could only ever return a row of ONE — which
// `rowRuns` has thrown away since 4.70 ("a row of one is not a row") and given
// its title back through `soloBar`. The whole apparatus composed exactly what
// `bar: STATS_BAR` with no row composes, and the shipped templates prove it:
// deleting it changed not one byte of any of the thirty-six fixtures.
//
// THE 4.70 ARGUMENT THAT MADE IT A TAB RATHER THAN A COLUMN IS NOT DELETED
// WITH IT, because it is about `stats-band` and still true wherever that
// section is put beside anything: it is ALREADY a horizontal band of four
// numbered cells dividing the width of its block, so halving that width wraps
// it into two rows of two. Any future pairing of it owes an answer to that.
//
// AND THE PAIRING ITSELF IS STILL AVAILABLE TO A READER. `stats` is
// `default: never`; somebody who wants the two boxes as one group makes it in
// *Edit sections…* with **Add to group**, which is the control 5.11 built for
// exactly this and which writes the same `row`/`tab` lines by hand. What is
// gone is the catalogue composing it unasked on a surface where the grid is no
// longer there to be paired with.
...
export const JOURNAL_SECTIONS: JournalSection[] = [
...

  {
    ...
    default: always,
    // ── AND BACK INSIDE THE BANNER'S FENCE AS OF 5.28 ─────────────────
    //
    // 4.20's argument, above, is about WHAT A BANNER MEANS, and it is still a
    // good one: the file's name, its navigation and the control that edits it,
    // and a rating is none of those. What it did not weigh is what the split
    // COSTS the page it is on, and three screenshots from a vault measured it:
    // a fresh Lesson spends 437px on chrome before the first word the reader
    // wrote, and 90px of that plus a whole card boundary is a two-field grid
    // wearing a head of its own.
    //
    // SO THE SECTION IS NOT UNDONE, ONLY ITS FENCE. Everything 4.20 moved out
    // of the banner stays out: this is its own entry in the catalogue, its own
    // row in *Edit sections…*, its own tick, and a reader may remove it or drag
    // it anywhere on the page — at which point it stops being welded and
    // composes its own card, bar and all. What 4.20 could not express was "one
    // card, two sections", and `weld` is that, on the shape the diary side has
    // called a band since 3.2 patch 2.
    //
    // AND THE RENDERER NEVER STOPPED DRAWING IT. `chromeClasses` says a fence
    // is a banner or it is the tracker section and that *"every note composed
    // before 4.20 keeps its logging grid inside whichever of the two its fence
    // is"*; `styles/30-header-bars.css` welds that grid to the card's lower
    // edge in seventy live lines. This release stops composing the shape those
    // two have supported all along only for notes nobody writes any more.
    //
    // THE DIARY ENTRY IS THE SAME CHANGE ONE CATALOGUE OVER and is deliberately
    // not made here — `entry-sections.ts` carries 4.20's split with the same
    // words, plus a footer case this banner has no equivalent of, and it goes
    // once this one has been looked at in a vault.
    claims: ["header", "tracker"],
    locate: (t) =>
      probe(t, new RegExp(`^(?:${TRACKER_MARK_START}|${LEGACY_TRACKER_MARK_START})\\s*$`, "m")),
    // THE TITLE IT WEARS WHEN IT IS NOT WELDED, which is the only time it has a
    // card of its own to title. `composeSectionRuns` filters this line out on
    // the way into the banner's fence — see the weld — so a reader who drags
    // the grid out of the banner gets it straight back.
    bar: TRACKERS_BAR,
    render: (ctx, opts) => [
      fence([
        TRACKERS_BAR,
...

  {
    ...
    // ITS OWN CARD AND ITS OWN TITLE, as of 5.28 — see the note above the
    // catalogue for the state row this used to be the second page of, and why
    // the grid moving into the banner ended it. `tab: true` went with it.
    bar: STATS_BAR,
    render: (_ctx, opts) => {
...

  {
    ...
    locked: false,
    default: always,
    // ── AND INTO THE BANNER'S FENCE TOO (5.28, second half) ────────────
    //
    // The reader saw the grid welded and the index still a card of its own,
    // with the chevron opening it four-and-twenty pixels below the banner it
    // was pressed on: *"trackers is ok but whats below is not attached to the
    // banner section."* So the note's index is welded on the same terms — one
    // card, its own section, its own tick.
    //
    // IT KEEPS ITS HEAD, WHICH THE GRID DOES NOT, and the difference is not an
    // inconsistency: the grid's head said "Trackers" over the note's own name,
    // which is the banner saying its own kind of thing twice, whereas this
    // section's head names something the banner does not — what is BELOW this
    // note, with the button that makes the first one in it. `soloBarOf` is what
    // the weld drops and this head is composed by `render`, so it survives by
    // construction rather than by exception.
    //
    // WHAT IT COSTS, AND THE READER CHOSE IT KNOWING: welded, this can no
    // longer be arranged below the writing, or dragged out of the group, and it
    // has no *Show as widget* box — a welded section composes no bar of its
    // own for the box to take off. Untick it, save, tick it again and it is a
    // card again, wherever the section list puts it.
    ...
    render: (ctx, opts) => {
      ...
      // The deepest index: its children are notes, so a table per kind rather
      // than a folder rollup.
      //
      // PER KIND rather than one combined table because the kinds are rated on
      // different things — a single table would need a column for every rating
      // in the type and leave most of it blank.
      //
      // ── AND 1.0.16 TESTED THAT SENTENCE TO DESTRUCTION ───────────────
      //
      // It composed ONE table over every kind, with a Type column naming which
      // kind each row was, on the reading that `kindTablePropertiesFor` answered
      // the objection above rather than denying it: the combined table carried a
      // column per DISTINCT rating, so nothing was duplicated.
      //
      // Rendered on the reader's own Topic index, over two Lessons: `Name | Type
      // | Date | Confidence | Accuracy | Status`. The Type column said "Lesson"
      // twice — one word repeated down the whole table — and Accuracy, the other
      // kind's rating, was an em dash on both rows. The objection was not
      // answered; it was moved from a table nobody had to read into a table
      // everybody does. The reader's verdict: *"the type field can be removed
      // entirely and reverted to the collapsible sections"*.
      //
      // WHAT WAS RIGHT ABOUT IT WENT UP A LEVEL. The cost it was aimed at —
      // heads and buttons and empty states multiplying with the kinds — is real
      // above the deepest level too, and worse, because there a column per kind
      // is paid for by every row. `folderRollup` draws one count column now.
      //
      // COMPOSED FROM `childrenParts`, which is the parts list itself (3.18
      // §1.3). One derivation, two readers: the renderer that writes the fence
      // and the planner that notices a fence is short of a part. Deriving them
      // separately is the drift that would let the plan report a gap filling it
      // could not close.
      //
      // UNDER ONE BAR AS OF 5.12 — see `childrenBar`. The parts are groups,
...
export function childrenParts(
  ...
  // ── IT WAS EMPTY OVER SEVERAL KINDS IN 1.0.16, AND IS NOT ──────────
  //
  // That release drew ONE table over every kind on the deepest index, so this
  // section had nothing repeatable left in it and returned no parts at all —
  // load-bearing at the time, because `missingParts` would otherwise have
  // offered to put the per-kind groups back on every freshly composed note.
  //
  // THE READER LOOKED AT IT: *"I think the implementation got things backwards.
  // the index pages above the deepest depth should gain a 'Notes' column for
  // the count, and the type field can be removed entirely and reverted to the
  // collapsible sections"*. The consolidation belonged one level up, where a
  // column per kind was empty on four rows of four (`folderRollup`), and not
  // here, where it cost a Type column repeating one word down every row.
  //
  // So the parts are back, and with them everything that hangs off being a
  // list: each kind's own head and chevron, its own create button beside its
  // own rows, its own empty state, and `extend` — which is how
  // `kind-change.ts`'s promise that *"Dashboards will offer to list the new
  // type"* is kept, and which had to be replaced by a whole migration while
  // there were no parts to be short of.
  //
...
// ── THE PROBE KNOWS ONE SPELLING AGAIN ───────────────────────────────
//
...
function kindTableProbe(ctx: SectionContext): RegExp {
  ...
  return new RegExp(`^\\s*${CHILDREN_TABLE}:(${ids.join("|")})\\s*$`, "m");
}
