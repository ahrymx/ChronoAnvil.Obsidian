...
**Everything the AGPL grants** — use it, read it, change it, redistribute it,
fork it, and sell it. Those are freedoms the licence exists to give and this
project means them.
...
3. **Mark a modified version as different** from the original, in a reasonable
   and prominent place.
4. **License your version under the AGPL-3.0**, as a whole.
...
mark their version as modified. That removes most of the incentive to resell
it, but it is a deterrent, not a prohibition, and this project would rather say
so plainly than imply an exclusivity it does not have.
...
marked as different, and the name "ChronoAnvil" is not licensed for use as the
name of your fork. A fork under its own name that credits its origin is welcome
and expected. A fork presenting itself as original work is a licence violation.

**Is there a commercial or proprietary licence?**
Not offered as standing terms. ChronoAnvil carried one briefly before release
and it was withdrawn as unnecessary: the AGPL already permits commercial use
and sale, so the only thing such a licence sells is relief from copyleft, and
no one has asked for it. As sole copyright holder, AhryMX can still grant
proprietary terms to anyone who has a real need — that right does not go away
by publishing under the AGPL. If you have such a need, write to
contact@ahrymx.dev and it can be discussed on its facts rather than
pre-negotiated in a document nobody has read.

**Our lawyers dislike the AGPL. What are our options?**
For internal use, there is nothing to dislike — no obligation is triggered. For
distribution, the options are to comply, to not distribute, or to write to
contact@ahrymx.dev and ask.

**Does the AGPL apply to my notes and vault content?**
No. Your notes are your data. A licence on a program does not reach the
documents you write with it.

**Can the AGPL grant be withdrawn later?**
Not for versions already published — that grant is irrevocable under the AGPL's
own terms. The copyright holder can choose different terms for future versions,
...
*This file is a summary written for readability. Where it differs from
[`LICENSE`](./LICENSE), the licence governs.*
