...
* **Product Name**:
  ...
  * `Almanac` is a FORMAT TOKEN and nothing else, as of 1.0.0. It survives in
    `tools/migrate-vault.mjs` and in the `almanac:` read-compatibility across
    ten files in `src/`, which is how a vault written under the old spelling
    still opens — `test/legacy-tokens.test.ts` is that contract. It appears in
    NO reader-facing document: not `LICENSE`, not `NOTICE`, not `LICENSING.md`,
    not `README.md`. It was provenance in those four until the history was reset
    for the 1.0.0 release, and the naming clause now lists one product name.
    `test/product-name.test.ts` sweeps all four for the word and requires none.
  * `ChronoForge` is gone from the tree, and must stay gone. It was the name
    ...
    only migration the tool performs is the old spelling's.
    `test/product-name.test.ts` sweeps that file along with the licence and
    README, so adding the dead name back by symmetry with `RULES` fails the
    suite.
